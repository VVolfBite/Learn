...
`asyncio` 是 Python 标准库中的一个模块，用于编写异步 I/O 操作代码。

`asyncio` 提供了一种高效的方式来处理并发任务，特别适用于 I/O 密集型操作，如网络请求、文件读写等。

通过使用 `asyncio`，可以在单线程中同时处理多个任务，而无需使用多线程或多进程。

在传统的同步编程中，当一个任务需要等待 I/O 操作（如网络请求）完成时，程序会阻塞，直到操作完成。这会导致程序的效率低下，尤其是在需要处理大量 I/O 操作时。

`asyncio` 通过引入异步编程模型，允许程序在等待 I/O 操作时继续执行其他任务，从而提高程序的并发性和效率。

`asyncio` 允许编写**单线程并发**代码，特别适用于网络爬虫、Web 服务器、微服务等 I/O 密集型场景。

`asyncio` 的核心是协程、事件循环、任务与 Future。
...
事件循环是 `asyncio` 的核心组件，负责调度和执行协程。它不断检查是否有任务需要执行，并在任务完成后调用相应逻辑。
...
`Future` 是一个表示异步操作结果的对象。它通常用于底层 API，表示一个尚未完成的操作。可以通过 `await` 关键字等待 `Future` 完成。
...
要运行一个协程，可以使用 `asyncio.run()` 函数。它会创建一个事件循环，并运行指定协程。

```python
asyncio.run(coroutineObject)
# coroutineObject: 顶层协程对象
# 返回值: coroutineObject 执行完成后的返回值
```
...
可以使用 `asyncio.gather()` 并发执行多个协程，并等待它们全部完成。

```python
await asyncio.gather(*awaitableObjectList, return_exceptions=False)
# awaitableObjectList: 多个可等待对象
# return_exceptions: 是否把异常作为结果返回
# 返回值: 结果列表
```
...
可以使用 `asyncio.wait_for()` 为协程设置超时时间。如果协程在指定时间内未完成，则抛出 `asyncio.TimeoutError`。

```python
await asyncio.wait_for(awaitableObject, timeoutValue)
# awaitableObject: 可等待对象
# timeoutValue: 超时时间
# 返回值: awaitableObject 的执行结果
```
...
任务取消通常通过 `task.cancel()` 发起。被取消的任务会在后续调度点抛出 `CancelledError`，因此通常需要配合异常处理一起使用。

```python
taskObject.cancel(msg=None)
# taskObject: Task 实例
# msg: 可选取消消息
# 返回值: bool
```
...
async def queue_producer(queueValue):
    for indexValue in range(3):
        await queueValue.put(indexValue)
        await asyncio.sleep(0.1)

async def queue_consumer(queueValue):
    for _ in range(3):
        itemValue = await queueValue.get()
        print(f"Consumed {itemValue}")
        queueValue.task_done()

async def main():
    lockValue = asyncio.Lock()
    eventValue = asyncio.Event()
    queueValue = asyncio.Queue()
    semaphoreValue = asyncio.Semaphore(2)

    async with lockValue:
        print("lock ready")

    eventValue.set()
    await eventValue.wait()

    async with semaphoreValue:
        producerTaskValue = asyncio.create_task(queue_producer(queueValue))
        consumerTaskValue = asyncio.create_task(queue_consumer(queueValue))
        await asyncio.gather(producerTaskValue, consumerTaskValue)
...
`asyncio` 特别适用于网络请求，如 HTTP 请求、WebSocket 通信、TCP 连接与服务器编写等场景。

```python
asyncio.open_connection(hostValue, portValue)
# hostValue: 主机地址
# portValue: 端口号
# 返回值: (readerObject, writerObject)
```

```python
asyncio.start_server(clientConnectedCallback, hostValue, portValue)
# clientConnectedCallback: 连接处理函数
# hostValue: 监听地址
# portValue: 监听端口
# 返回值: Server 对象
```
...
`asyncio` 也支持异步创建和管理子进程。

```python
asyncio.create_subprocess_exec(programValue, *argumentValue)
# programValue: 可执行程序
# argumentValue: 命令行参数
# 返回值: Process 对象
```

- **Python 版本：**部分功能需 Python 3.7+，如 `asyncio.run()`。
- **阻塞操作：**避免在协程中使用同步阻塞代码，如 `time.sleep()`。
- **调试：**设置 `PYTHONASYNCIODEBUG=1` 环境变量可启用调试模式。
- **取消任务：**被取消的任务会引发 `CancelledError`，需要妥善处理。

#### threading 模块

Python 的 `threading` 模块是用于实现多线程编程的标准库之一。多线程允许程序在同一时间内执行多个任务，从而提高程序的效率和响应速度。

`threading` 模块提供了创建和管理线程的工具，使得开发者可以轻松编写并发程序。

可以把线程想象成办公室里的员工：

- **单线程程序：**就像只有一个员工，他必须顺序完成打印文档、回复邮件、泡咖啡等所有工作。
- **多线程程序：**则像拥有多个员工，他们可以同时进行不同任务，大大提高工作效率。

在计算机科学中：

- **进程：**一个运行中的程序，拥有独立的内存空间，例如同时打开的浏览器和音乐播放器就是两个进程。
- **线程：**进程内的一个独立执行流，是 CPU 调度的基本单位。同一个进程内的所有线程共享该进程的内存空间，如全局变量。

在单线程程序中，任务是一个接一个顺序执行的。如果某个任务需要等待，例如等待网络响应或文件读取，整个程序会被阻塞，直到该任务完成。而多线程可以让程序在等待某个任务的同时继续执行其他任务，从而提高程序整体性能。
...
```python
threadObject = threading.Thread(target=functionObject, args=(argumentValue,))
...
1. **继承 threading.Thread 类：**通过重写 `run()` 方法实现线程逻辑。
...
2. **使用 threading.Thread 构造函数：**直接指定目标函数。
...
```python
threadObject.start()
# threadObject: Thread 实例
# 返回值: None
```

```python
threadObject.join(timeout=None)
# threadObject: Thread 实例
# timeout: 最长等待时间
# 返回值: None
```
...
threadListValue = []
for indexValue in range(3):
    threadValue = threading.Thread(target=worker, args=(indexValue,))
    threadListValue.append(threadValue)
    threadValue.start()

for threadValue in threadListValue:
    threadValue.join()
    print(threadValue.name)
    print(threadValue.is_alive())
    print(threadValue.ident)
...
在多线程编程中，多个线程可能会同时访问共享资源，这可能导致数据不一致问题。为了避免这种情况，可以使用线程同步机制，例如锁（`Lock`）。
...
# lock: 关联的 Lock 或 RLock；省略时自动创建 RLock
# 返回值: Condition 对象
```

```python
conditionObject.wait(timeout=None)
# conditionObject: Condition 实例
# timeout: 超时时间
# 返回值: None
```

```python
conditionObject.notify(n=1)
# conditionObject: Condition 实例
# n: 唤醒线程数量
# 返回值: None
```
...
```python
eventObject.set()
# eventObject: Event 实例
# 返回值: None
```

```python
eventObject.wait(timeout=None)
# eventObject: Event 实例
# timeout: 超时时间
# 返回值: bool
```
...
```python
threading.Timer(intervalValue, functionObject, args=None, kwargs=None)
# intervalValue: 延迟执行秒数
# functionObject: 到时后执行的函数
# args: 位置参数
# kwargs: 关键字参数
# 返回值: Timer 对象
```
...
lockValue = threading.Lock()
eventValue = threading.Event()
conditionValue = threading.Condition()
semaphoreValue = threading.Semaphore(2)

def worker():
    with lockValue:
        print("lock protected")
...
timerValue = threading.Timer(1.0, print, args=("timer fired",))
timerValue.start()
```

##### 线程通信

线程间通信可以通过队列（`Queue`）实现。`Queue` 是线程安全的，可以在多个线程之间安全传递数据。
...
localValue = threading.local()

def producer():
    for indexValue in range(3):
        queueValue.put(indexValue)

def consumer():
    localValue.threadName = threading.current_thread().name
    while not queueValue.empty():
        itemValue = queueValue.get()
        print(localValue.threadName, itemValue)
        queueValue.task_done()
...
- **线程安全：**在多线程环境中，需要确保对共享资源的访问是线程安全的，避免数据竞争和死锁。
- **线程数量：**创建过多线程可能导致系统资源耗尽，影响程序性能。应合理控制线程数量，或使用线程池（`ThreadPoolExecutor`）来管理线程。

- ### GIL（全局解释器锁）

  Python 的多线程并发执行并不像其他编程语言那样能在多核处理器上实现真正的并行执行。这是因为 Python 的 **全局解释器锁（GIL）** 限制了同一时刻只有一个线程执行 Python 字节码。

  #### GIL 的影响

  - **CPU密集型任务：**由于 GIL，Python 的线程无法在多个核心上并行执行计算密集型任务。即便使用多线程，Python 也无法利用多核 CPU 的优势。
  - **I/O 密集型任务：**GIL 对于 I/O 密集型任务影响较小，因为在 I/O 操作（例如文件读取、网络请求）时，GIL 被释放，其他线程可以执行。这样，Python 线程仍然可以有效地执行 I/O 操作。

  #### GIL 示例

  下面这个例子用于说明：对于纯 Python 计算循环这类 CPU 密集型任务，即便创建多个线程，也不意味着能获得真正的多核并行计算收益。这个示例更适合用来理解 GIL 对 CPU 密集型任务的限制，而不是用于说明 I/O 并发收益。
  ...
  threadListValue = []
  for indexValue in range(10):
      threadValue = threading.Thread(target=count)
      threadListValue.append(threadValue)
      threadValue.start()
  ...
  print("Total time:", time.time() - startTimeValue)
  ```

  在这个示例中，尽管我们创建了多个线程，但由于 GIL 的存在，计算任务实际上并未在多个核心上并行执行。
...
作用域遵循 **LEGB**。作用域就是一个 Python 程序可以直接访问命名空间的正文区域。在一个 Python 程序中，直接访问一个变量时，会从内到外依次访问所有作用域，直到找到该名称；如果最终没有找到，则会报未定义错误。

Python 中，程序的变量并不是在任何位置都可以访问的，访问权限取决于这个变量是在哪里赋值的。变量的作用域决定了程序的哪一部分可以访问哪个特定变量名称。

Python 的作用域一共有 4 种，分别是：

- **L（Local）**：最内层作用域，包含局部变量，比如函数或方法内部的变量。
- **E（Enclosing）**：外层嵌套作用域，包含非局部（`nonlocal`）且非全局（`non-global`）的变量。比如嵌套函数中，外层函数的局部作用域对于内层函数来说就是 enclosing 作用域。
- **G（Global）**：当前模块最外层作用域，比如当前模块中的全局变量。
- **B（Built-in）**：内建作用域，包含内建名称、内建函数、关键字相关名称等，最后被搜索。

- 名称查找顺序：**L -> E -> G -> B**。

Python 中，只有**模块（module）**、**类（class）**以及**函数（def / lambda）**会引入新的作用域，其它代码块（如 `if / elif / else`、`try / except`、`for / while` 等）不会引入新的作用域。也就是说，这些语句块中定义的变量，外部通常仍然可以访问。

定义在函数内部的变量拥有局部作用域，定义在函数外部的变量拥有全局作用域。局部变量只能在其声明所在的函数内部访问，而全局变量可以在整个模块范围内访问。函数调用时，其内部变量会被加入该函数的局部作用域中，并且不会影响函数外部的同名变量。

```python
totalValue = 0  # global scope name

def sum_value(argumentValue1, argumentValue2):
    # local scope name
    totalValue = argumentValue1 + argumentValue2
    print("函数内是局部变量:", totalValue)
    return totalValue

sum_value(10, 20)
print("函数外是全局变量:", totalValue)
```

当内部作用域想修改全局作用域中的变量时，需要使用 `global` 关键字。

1. **global：**声明当前函数内使用的是模块级全局变量，而不是当前局部变量。

```python
global nameValue
# nameValue: 模块级全局名称
# 说明: 在函数内部对该名称进行赋值时，使其绑定到全局作用域
```
...
如果要修改嵌套作用域（enclosing 作用域，即外层非全局作用域）中的变量，则需要使用 `nonlocal` 关键字。

1. **nonlocal：**声明当前函数内使用的是外层嵌套函数作用域中的变量，而不是当前局部变量。

```python
nonlocal nameValue
# nameValue: 外层嵌套函数作用域中的名称
# 说明: 在内层函数中重新绑定外层非全局变量
```
...
Python 提供了一种机制，可以将函数、类、变量和可执行代码存放在单个 `.py` 文件中，这个文件被称为模块（Module）。模块可以被脚本或交互式解释器实例导入和使用，以便将代码组织成可重用单元，方便管理和维护。

每个模块都有一个内置属性 `__name__`：

- 如果模块是直接运行的入口文件，`__name__` 的值为 `"__main__"`。
- 如果模块被导入，`__name__` 的值为模块名，通常是文件名（不含 `.py` 后缀）或包路径形式。

内置函数 `dir()` 可以返回模块内定义的所有名称，并以字符串列表形式呈现。
...
1. **__name__：**表示当前模块的名称或当前模块的运行身份。

```python
__name__
# 返回值:
# 1. 作为主程序运行时，值为 "__main__"
# 2. 作为模块被导入时，值为模块名或包路径名
```

2. **dir：**返回当前对象或模块中可访问的名称列表。

```python
dir(objectValue)
# objectValue: 可选参数；省略时返回当前局部作用域名称列表
# 返回值: list[str]
```
...
包（Package）是管理 Python 模块命名空间的一种形式，采用“点模块名称”的方式组织模块。例如模块名 `package1.module2` 表示包 `package1` 中的子模块 `module2`。

包可以防止不同模块或库的全局变量冲突以及模块重名问题。

在导入包时，Python 会根据 `sys.path` 中的路径寻找包目录。目录中通常包含 `__init__.py`，该文件会在包被导入时执行，可以包含初始化代码，也可以定义 `__all__` 列表变量，用于控制 `from package import *` 导入的内容。

1. **__init__.py：**包初始化文件，在包导入时执行。
...
# 1. 用于标识当前目录作为包
# 2. 可包含包初始化逻辑
# 3. 可定义 __all__ 等导出控制内容
```

2. **__all__：**控制 `from package import *` 时导入的名称列表。

```python
__all__ = ["nameValue1", "nameValue2"]
# nameValue1, nameValue2: 允许被 import * 导入的名称
```
...
1. **import：**导入整个模块或包，之后通过 `module.name` 的形式访问其中的名称。
...
# moduleName: 模块名或包名
# aliasName: 导入别名
# 说明: 导入后通过 moduleName.name 或 aliasName.name 访问成员
```

2. **from ... import ...：**从模块或包中导入指定名称到当前命名空间，之后可直接使用该名称。

```python
from moduleName import nameValue
from moduleName import nameValue as aliasName
# moduleName: 模块名或包名
# nameValue: 导入的名称
# aliasName: 导入名称的别名
# 说明: 导入后可直接使用 nameValue 或 aliasName
```

3. **相对导入：**基于当前模块所在包层级进行解析。
...
4. **绝对导入：**从顶层包或 `sys.path` 路径起始位置进行解析。
...
导入解析遵循模块搜索路径 `sys.path`：第一个匹配的包或模块会被认为是目标导入对象。相对导入使用 `.` / `..`，基于当前模块所在包层级；绝对导入以顶层包或 `sys.path` 路径起始位置进行解析。

注意，导入是相对包而言的，而不是相对模块文件路径而言的。
...
绝对导入路径从顶层包开始，相对导入路径基于模块所在包。`__name__` 为 `"__main__"` 的模块作为主程序入口时，主模块通常应使用绝对导入路径引用。

### Lambda 表达式

Python 使用 **lambda** 来创建匿名函数。`lambda` 函数是一种小型、匿名的、内联函数，它可以具有任意数量的参数，但只能有一个表达式。匿名函数不需要使用 `def` 关键字定义完整函数。

`lambda` 函数通常用于编写简单的、单行的函数，常见于需要把函数作为参数传递的场景，例如 `map()`、`filter()`、`reduce()` 等函数中。

**lambda 函数特点：**

- `lambda` 函数是匿名的，它们没有函数名称，通常通过赋值给变量或作为参数传递给其他函数来使用。
- `lambda` 函数通常只包含一行表达式，因此更适合编写简单函数。
- `lambda` 函数体中只能写表达式，不能直接写多条语句。

**lambda 语法格式：**

```python
lambda argumentList: expressionValue
# argumentList: 参数列表，可以有零个或多个参数
# expressionValue: 单个表达式，表达式结果会被直接返回
# 返回值: lambda function object
```

- `lambda` 是 Python 的关键字，用于定义匿名函数。
- `argumentList` 是参数列表，可以包含零个或多个参数，但必须写在冒号 `:` 前面。
- `expressionValue` 是一个表达式，用于计算并返回函数结果。

`lambda` 函数也可以设置多个参数，参数之间使用逗号 `,` 分隔。`lambda` 函数通常与内置函数如 `map()`、`filter()` 和 `reduce()` 一起使用，以便在集合上执行操作。

1. **lambda：**定义匿名函数表达式。

```python
lambda argumentList: expressionValue
# argumentList: lambda 形参列表
# expressionValue: 函数体中的单个表达式
# 返回值: function object
# 说明: 调用该 function object 时，返回 expressionValue 的计算结果
```

2. **lambda 赋值：**把匿名函数绑定到变量名，再像普通函数一样调用。

```python
functionValue = lambda leftValue, rightValue: leftValue + rightValue
# functionValue: 保存 lambda function object 的变量
# leftValue, rightValue: 位置参数
# leftValue + rightValue: 返回值表达式
# 返回值: functionValue(...) 的调用结果
```

例如，下面把一个 `lambda` 表达式赋值给变量，再直接调用：

```python
addValue = lambda leftValue, rightValue: leftValue + rightValue
# addValue: 保存匿名函数对象
# leftValue, rightValue: 两个位置参数
# leftValue + rightValue: 返回两个参数相加后的结果

print(addValue(10, 20))
```

3. **lambda 作为参数：**把匿名函数作为可调用对象传递给其它函数。

```python
callableObject = lambda argumentValue: expressionValue
# callableObject: 作为参数传递的可调用对象
# argumentValue: lambda 形参
# expressionValue: 返回值表达式
# 返回值: callableObject(...) 的调用结果
```

例如，排序、自定义映射、筛选等场景中，通常会把 `lambda` 作为参数传给高阶函数。

4. **map：**把函数作用到可迭代对象的每个元素上。

```python
map(functionObject, iterableObject, *iterableObjectList)
# functionObject: 用于处理元素的函数对象或 lambda function object
# iterableObject: 第一个可迭代对象
# iterableObjectList: 其它可迭代对象
# 返回值: map object
# 说明: 需要再用 list(...) 等方式取出结果
```

5. **filter：**按条件筛选可迭代对象中的元素。

```python
filter(functionObject, iterableObject)
# functionObject: 返回布尔值的函数对象或 lambda function object
# iterableObject: 被筛选的可迭代对象
# 返回值: filter object
# 说明: 只保留 functionObject(itemValue) 为 True 的元素
```

6. **reduce：**把可迭代对象中的元素累计归约为单个结果。

`reduce()` 位于 `functools` 模块中，使用前通常需要先导入。

```python
functools.reduce(functionObject, iterableObject[, initializerValue])
# functionObject: 接收两个参数的函数对象或 lambda function object
# iterableObject: 参与归约计算的可迭代对象
# initializerValue: 可选初始值；省略时通常从前两个元素开始计算
# 返回值: 累计归约后的单个结果
```

下面使用一个统一示例把 `lambda`、`map()`、`filter()` 和 `reduce()` 串起来：

```python
import functools

addValue = lambda leftValue, rightValue: leftValue + rightValue
# addValue: 接收两个参数并返回它们的和

squareValue = lambda itemValue: itemValue * itemValue
# squareValue: 接收一个参数并返回平方结果

evenCheckValue = lambda itemValue: itemValue % 2 == 0
# evenCheckValue: 判断当前元素是否为偶数，返回 bool 值

print(addValue(10, 20))
# 调用 addValue，输出 30

numberListValue = [1, 2, 3, 4, 5, 6]
# numberListValue: 被处理的原始列表

mapResultValue = map(squareValue, numberListValue)
# mapResultValue: map object
# 作用: 对列表中每个元素执行平方运算
print(list(mapResultValue))

filterResultValue = filter(evenCheckValue, numberListValue)
# filterResultValue: filter object
# 作用: 只保留列表中的偶数元素
print(list(filterResultValue))

reduceResultValue = functools.reduce(
    lambda leftValue, rightValue: leftValue + rightValue,
    numberListValue,
)
# reduceResultValue: int
# 作用: 把列表元素依次累加为单个结果
print(reduceResultValue)
```
...
装饰器（decorators）是 Python 中的一种高级功能，它允许动态地修改函数或类的行为。装饰器本质上是一个可调用对象，它接收一个函数或类作为参数，并返回一个新的函数、类，或者对原对象进行包装后的结果。

装饰器的核心价值在于：**不修改原有函数或类定义的前提下，为其增加额外功能**，例如日志记录、权限校验、性能统计、缓存、单例控制等。

![img](./assets/decorators-python-1-1774600094054-1.png)

#### 函数装饰器

装饰器的语法使用 `@decoratorName`，放在函数或方法定义之前。

Python 还提供了一些内置装饰器，比如 `@staticmethod` 和 `@classmethod`，用于定义静态方法和类方法。Python 装饰器允许在不修改原有函数代码的基础上，动态地增加或修改函数功能。装饰器本质上是一个接收函数作为输入并返回一个新的包装后函数的对象。

可以把函数装饰器理解成下面这种结构：

- `decoratorFunction` 是装饰器，它接收一个函数 `originalFunction` 作为参数。
- `wrapperFunction` 是内部包装函数，它才是最终真正被调用的新函数。
- 当使用 `@decoratorFunction` 修饰 `targetFunction` 时，Python 会自动把 `targetFunction` 传给 `decoratorFunction`，并用返回值替换原来的 `targetFunction`。
...
targetFunction = timeLogger(targetFunction)
```

这会将 `targetFunction` 函数传递给 `timeLogger` 装饰器，并将返回的新函数重新赋值给 `targetFunction`。因此，后续每次调用 `targetFunction()` 时，实际上调用的是经过装饰器包装后的函数。

下面是一个最基础的函数装饰器结构：

```python
def logExecutionDecorator(originalFunction):
    # originalFunction: 被装饰的原始函数对象

    def wrapperFunction(*argsValue, **kwargsValue):
        # wrapperFunction: 实际替代 originalFunction 的包装函数
        # *argsValue: 接收原函数的全部位置参数
        # **kwargsValue: 接收原函数的全部关键字参数

        print("调用目标函数前执行日志逻辑")

        resultValue = originalFunction(*argsValue, **kwargsValue)
        # resultValue: 保存原始函数执行后的返回结果
...
@logExecutionDecorator
def runBusinessAction():
    # runBusinessAction: 被装饰的目标函数
    print("执行核心业务逻辑")
...
如果原函数需要参数，则在装饰器的 `wrapperFunction` 中继续接收并传递参数即可。

例如，下面是一个支持参数的函数装饰器：

```python
def logCallDecorator(originalFunction):
    # originalFunction: 被装饰的函数对象

    def wrapperFunction(*argsValue, **kwargsValue):
        # *argsValue / **kwargsValue: 接收调用时传入的全部参数
        print(f"调用函数: {originalFunction.__name__}")
        print(f"位置参数: {argsValue}")
        print(f"关键字参数: {kwargsValue}")

        resultValue = originalFunction(*argsValue, **kwargsValue)
        # resultValue: 原始函数执行后的返回值

        print(f"返回结果: {resultValue}")
        return resultValue
...
@logCallDecorator
def addNumberValue(leftNumberValue, rightNumberValue):
    # leftNumberValue, rightNumberValue: 两个参与计算的位置参数
    return leftNumberValue + rightNumberValue


print(addNumberValue(10, 20))
```

通过装饰器，开发者可以在保持代码整洁的同时，灵活且高效地扩展程序功能。
...
- **函数形式的类装饰器**：接收类作为参数，返回新的类或包装类
- **类形式的类装饰器**：实现 `__call__()` 方法，使实例本身可调用

下面是一个函数形式的类装饰器：

```python
def wrapDisplayClassDecorator(originalClass):
    # originalClass: 被装饰的原始类对象

    class WrappedDisplayClass:
        # WrappedDisplayClass: 返回的包装类，用于替代原始类

        def __init__(self, *argsValue, **kwargsValue):
            # *argsValue / **kwargsValue: 创建原始类实例时传入的参数
            self.wrappedInstanceValue = originalClass(*argsValue, **kwargsValue)
            # wrappedInstanceValue: 保存真正的原始类实例

        def __getattr__(self, attributeNameValue):
            # __getattr__: 当前包装类中找不到属性时，转发给原始实例
            # attributeNameValue: 被访问的属性名称
            return getattr(self.wrappedInstanceValue, attributeNameValue)

        def displayProfileInfo(self):
            # displayProfileInfo: 对原始类中的 displayProfileInfo 方法进行增强
            print(f"调用 {originalClass.__name__}.displayProfileInfo() 前")
            self.wrappedInstanceValue.displayProfileInfo()
            print(f"调用 {originalClass.__name__}.displayProfileInfo() 后")

    return WrappedDisplayClass
    # 返回包装类，用于替换原始类


@wrapDisplayClassDecorator
class UserProfile:
    def displayProfileInfo(self):
        print("输出用户资料展示信息")
...
userProfileObject.displayProfileInfo()
```

上面这个例子中：

- `wrapDisplayClassDecorator` 接收原始类 `UserProfile`
- `WrappedDisplayClass` 是新的包装类
- 创建 `UserProfile()` 时，实际上返回的是 `WrappedDisplayClass` 的实例
- `WrappedDisplayClass` 内部再去创建真正的原始类实例，并对方法调用进行增强

#### 内置装饰器

Python 提供了一些常用的内置装饰器，例如：
...
class UserProfileRecord:
    @staticmethod
    def printStaticDescription():
        # printStaticDescription: 静态方法
        # 特点: 不依赖实例对象，也不依赖类对象
        print("这是一个静态方法示例")

    @classmethod
    def printClassDescription(cls):
        # cls: 当前类对象
        # printClassDescription: 类方法
        # 特点: 第一个参数默认接收类本身
        print(f"这是一个属于 {cls.__name__} 的类方法示例")

    @property
    def userNameValue(self):
        # userNameValue: 读取型属性方法
        # self._userNameValue: 实际保存数据的实例属性
        return self._userNameValue

    @userNameValue.setter
    def userNameValue(self, inputUserNameValue):
        # inputUserNameValue: 外部赋给属性的新值
        # @userNameValue.setter: 为 property 补充赋值逻辑
        self._userNameValue = inputUserNameValue


UserProfileRecord.printStaticDescription()
UserProfileRecord.printClassDescription()

userProfileRecordObject = UserProfileRecord()
userProfileRecordObject.userNameValue = "AliceProfile"
print(userProfileRecordObject.userNameValue)
```

其中：

- `@staticmethod`：把方法定义为静态方法，不会自动传入 `self` 或 `cls`
- `@classmethod`：把方法定义为类方法，第一个参数通常是 `cls`
- `@property`：把方法伪装成属性访问形式，使其可以通过 `objectValue.attributeName` 的方式读取
- `@property.setter`：为属性补充赋值逻辑

装饰器本质上是一种“包装”思想。函数装饰器主要用于包装函数调用过程，类装饰器主要用于包装类或实例化过程，而内置装饰器则提供了一些常见对象模型能力的语法支持。

### 迭代器与生成器

#### 迭代器

迭代是 Python 最强大的功能之一，是访问集合元素的一种方式。迭代器是一个可以记住遍历位置的对象。迭代器对象从集合的第一个元素开始访问，直到所有元素被访问完结束。迭代器只能向前移动，不能后退。

迭代器有两个基本方法：`iter()` 和 `next()`。字符串、列表、元组等可迭代对象都可以用来创建迭代器。迭代器对象既可以使用常规 `for` 语句进行遍历，也可以使用 `next()` 函数逐个获取元素。

```python
numberListValue = [1, 2, 3, 4]
numberIteratorValue = iter(numberListValue)
# numberIteratorValue: 由列表对象创建出的 iterator object

for itemValue in numberIteratorValue:
    # itemValue: 每次迭代取得的当前元素
    print(itemValue, end=" ")
```

除了 `for` 循环，也可以手动调用 `next()` 逐个取值。迭代结束时会触发 `StopIteration` 异常。
...
while True:
    try:
        currentItemValue = next(numberIteratorValue)
        # currentItemValue: 当前通过 next(...) 取得的元素
        print(currentItemValue)
    except StopIteration:
        # StopIteration: 表示迭代器中已经没有更多元素
        sys.exit()
```

##### 创建迭代器

如果要把一个类本身作为迭代器使用，则通常需要在类中实现 `__iter__()` 与 `__next__()` 两个方法。

- `__iter__()`：返回迭代器对象本身或其它迭代器对象。
- `__next__()`：返回下一个元素；如果没有更多元素，则抛出 `StopIteration`。

如果已经了解面向对象编程，就会知道类通常有构造函数 `__init__()`，它会在对象初始化时执行。自定义迭代器时，经常会在 `__init__()` 或 `__iter__()` 中初始化迭代状态。

```python
class NumberRangeIterator:
    def __iter__(self):
        # self.currentNumberValue: 当前迭代位置
        self.currentNumberValue = 1
        return self
        # 返回当前对象本身作为 iterator object

    def __next__(self):
        if self.currentNumberValue <= 20:
            nextNumberValue = self.currentNumberValue
            self.currentNumberValue += 1
...
numberRangeObject = NumberRangeIterator()
numberRangeIteratorValue = iter(numberRangeObject)

for itemValue in numberRangeIteratorValue:
    print(itemValue)
```

`__next__()` 方法会返回下一个迭代结果。`StopIteration` 异常用于标识迭代结束，防止出现无限循环。在 `__next__()` 方法中，通常会在满足终止条件后主动抛出该异常。

#### 生成器

在 Python 中，使用了 `yield` 的函数被称为生成器（generator）。`yield` 是一个关键字，用于定义生成器函数。生成器函数是一种特殊函数，可以在迭代过程中逐步产生值，而不是一次性返回所有结果。

与普通函数不同，生成器函数调用后不会立即执行完整函数体，而是返回一个生成器对象。生成器对象本质上也是一个迭代器，因此可以用于 `for` 循环或 `next()` 逐个取值。

当在生成器函数中使用 `yield` 语句时，函数执行会暂停，并把 `yield` 后面的表达式作为当前迭代值返回。之后每次再次调用 `next()` 或继续迭代时，函数会从上次暂停的位置继续执行，直到再次遇到 `yield` 或执行结束。

```python
def countdownGenerator(startNumberValue):
    # startNumberValue: 倒计时起始值
    while startNumberValue > 0:
        yield startNumberValue
        # yield startNumberValue: 产出当前值，并暂停函数执行
        startNumberValue -= 1
        # 下一次恢复执行时，从这里继续向下运行


countdownGeneratorObject = countdownGenerator(5)
# countdownGeneratorObject: generator object

print(next(countdownGeneratorObject))  # 输出: 5
print(next(countdownGeneratorObject))  # 输出: 4
print(next(countdownGeneratorObject))  # 输出: 3

for itemValue in countdownGeneratorObject:
    # 此时会从上次暂停的位置继续迭代
    print(itemValue)  # 输出: 2 1
```

可以把生成器理解为一种“按需产出数据”的迭代器实现方式。它不需要一次性构造完整结果，因而在处理大数据流、惰性计算、逐步生成结果时非常有用。

### 正则表达式

正则表达式是一个特殊的字符序列，它能帮助你方便地检查一个字符串是否与某种模式匹配。在 Python 中，使用 `re` 模块来处理正则表达式。`re` 模块提供了一组函数，允许你在字符串中进行模式匹配、搜索和替换操作，因此 Python 具备了完整的正则表达式处理能力。
...
- 批量查找所有匹配结果
- 按匹配规则进行替换或切分
- 先编译模式，再复用 Pattern 对象提高可读性和复用性

`re.compile()` 返回 `RegexObject`（也就是 Pattern 对象），匹配成功后返回 `MatchObject`。

**RegexObject：**

`re.compile()` 返回的对象就是正则表达式编译对象，它可以继续调用 `match()`、`search()`、`findall()`、`finditer()` 等方法。

**MatchObject：**

匹配成功后返回 `MatchObject`，可以通过它获取匹配结果及位置信息，例如：
...
1. **match：**从字符串起始位置开始匹配模式；如果不是从起始位置匹配成功，则返回 `None`。

```python
re.match(patternValue, stringValue, flags=0)
# patternValue: 正则表达式模式字符串
# stringValue: 待匹配的原始字符串
# flags: 可选修饰符，控制匹配方式
# 返回值: MatchObject | None
```

| 参数           | 描述                                 |
| :------------- | :----------------------------------- |
| `patternValue` | 匹配使用的正则表达式                 |
| `stringValue`  | 要执行匹配的字符串                   |
| `flags`        | 标志位，用于控制正则表达式的匹配方式 |

2. **search：**扫描整个字符串，并返回第一个成功匹配的结果。

```python
re.search(patternValue, stringValue, flags=0)
# patternValue: 正则表达式模式字符串
# stringValue: 待匹配的原始字符串
# flags: 可选修饰符，控制匹配方式
# 返回值: MatchObject | None
...
3. **sub：**把字符串中匹配到的内容替换为指定值。

```python
re.sub(patternValue, replacementValue, stringValue, count=0, flags=0)
# patternValue: 正则表达式模式字符串
# replacementValue: 替换内容，也可以是函数对象
# stringValue: 原始字符串
...
4. **compile：**编译正则表达式，生成可复用的 Pattern 对象。

```python
re.compile(patternValue, flags=0)
# patternValue: 字符串形式的正则表达式
# flags: 可选修饰符，控制匹配方式
# 返回值: RegexObject
```

5. **findall：**查找所有匹配到的子串，并返回列表。

```python
re.findall(patternValue, stringValue, flags=0)
patternObject.findall(stringValue, pos=0, endpos=len(stringValue))
# patternValue: 正则表达式模式字符串
# patternObject: 通过 re.compile(...) 得到的 RegexObject
# stringValue: 待匹配的字符串
# pos: 起始位置
# endpos: 结束位置
# 返回值: list
...
**注意：**`match()` 和 `search()` 通常返回一次匹配结果，而 `findall()` 会返回全部匹配结果。

6. **finditer：**查找所有匹配结果，并把它们以迭代器形式返回。

```python
re.finditer(patternValue, stringValue, flags=0)
# patternValue: 正则表达式模式字符串
# stringValue: 待匹配的字符串
# flags: 可选修饰符
# 返回值: iterator[MatchObject]
```
...
7. **split：**按照能够匹配的子串对字符串进行切分。

```python
re.split(patternValue, stringValue, maxsplit=0, flags=0)
# patternValue: 正则表达式模式字符串
# stringValue: 待切分的字符串
# maxsplit: 最大切分次数，0 表示不限制
# flags: 可选修饰符
# 返回值: list[str]
```
...
匹配成功时会返回 `MatchObject`。通过匹配对象可以进一步获取分组内容和位置信息。

1. **group：**返回匹配到的整个字符串，或指定分组对应的字符串。

```python
matchObject.group(groupIndex=0)
# matchObject: MatchObject
# groupIndex: 分组编号；0 表示整个匹配结果
# 返回值: str | tuple[str, ...]
```

2. **groups：**返回所有分组结果组成的元组。
...
3. **start：**返回匹配结果在原字符串中的起始位置。

```python
matchObject.start(groupIndex=0)
# matchObject: MatchObject
# groupIndex: 分组编号；默认 0
# 返回值: int
```

4. **end：**返回匹配结果在原字符串中的结束位置。

```python
matchObject.end(groupIndex=0)
# matchObject: MatchObject
# groupIndex: 分组编号；默认 0
# 返回值: int
```

5. **span：**返回匹配结果的位置范围元组。

```python
matchObject.span(groupIndex=0)
# matchObject: MatchObject
# groupIndex: 分组编号；默认 0
# 返回值: tuple[int, int]
```
...
正则表达式可以包含一些可选标志修饰符，用来控制匹配模式。它们既可以单独使用，也可以通过按位或 `|` 组合使用。

| 修饰符                    | 描述                                    | 示例                                   |
| :------------------------ | :-------------------------------------- | :------------------------------------- |
| `re.IGNORECASE` 或 `re.I` | 使匹配对大小写不敏感                    | `re.compile(r"apple", flags=re.I)`     |
| `re.MULTILINE` 或 `re.M`  | 多行匹配，影响 `^` 和 `$` 的行为        | `re.compile(r"^\d+", flags=re.M)`      |
| `re.DOTALL` 或 `re.S`     | 使 `.` 匹配包括换行符在内的任意字符     | `re.compile(r"a.b", flags=re.S)`       |
| `re.ASCII`                | 使 `\w`、`\d`、`\s` 等只匹配 ASCII 字符 | `re.compile(r"\w+", flags=re.ASCII)`   |
| `re.VERBOSE` 或 `re.X`    | 忽略空格和注释，便于组织复杂正则        | `re.compile(r"...", flags=re.X)`       |
| `re.L`                    | 使部分字符类依赖当前区域设置环境        | `re.compile(patternValue, flags=re.L)` |

这些标志可以单独使用，也可以组合使用，例如：

```python
re.compile(patternValue, flags=re.IGNORECASE | re.MULTILINE)
...
模式字符串使用特殊语法表示正则表达式。字母和数字通常表示它们自身；很多特殊符号在正则中具有特定含义。由于正则表达式通常包含较多反斜杠，实际书写时一般推荐使用原始字符串，例如 `r"\d+"`。

| 模式          | 描述                                                      |
| :------------ | :-------------------------------------------------------- |
| `^`           | 匹配字符串开头                                            |
| `$`           | 匹配字符串结尾                                            |
| `.`           | 匹配除换行符外的任意字符；指定 `re.DOTALL` 时也可匹配换行 |
| `[...]`       | 匹配字符集合中的任意一个字符                              |
| `[^...]`      | 匹配不在字符集合中的任意一个字符                          |
| `re*`         | 匹配前一个表达式 0 次或多次                               |
| `re+`         | 匹配前一个表达式 1 次或多次                               |
...
下面先用一个示例对比 `match()` 与 `search()`：

```python
import re

sourceTextValue = "contact: support@example.com"

matchResultValue = re.match(r"support", sourceTextValue)
# matchResultValue: None
# 说明: match(...) 只会从字符串起始位置开始尝试匹配

searchResultValue = re.search(r"support", sourceTextValue)
# searchResultValue: MatchObject
# 说明: search(...) 会扫描整个字符串，找到第一个成功匹配

print(matchResultValue)
print(searchResultValue.group() if searchResultValue else None)
```

下面再用一个统一示例串起 `compile()`、`findall()`、`finditer()`、`split()`、`sub()` 以及 `MatchObject` 的常用方法：

```python
import re

sourceTextValue = "Order-101, Order-202, Order-303"
patternObject = re.compile(r"Order-(\d+)", flags=re.IGNORECASE)
# patternObject: RegexObject
# r"Order-(\d+)": 匹配订单号，并把数字部分放入第 1 个分组
# flags=re.IGNORECASE: 忽略大小写

searchResultValue = patternObject.search(sourceTextValue)
# searchResultValue: 第一个成功匹配到的 MatchObject

if searchResultValue:
    print(searchResultValue.group(0))
    # group(0): 整个匹配结果，例如 "Order-101"

    print(searchResultValue.group(1))
    # group(1): 第 1 个分组结果，例如 "101"

    print(searchResultValue.groups())
    # groups(): 返回所有分组组成的元组

    print(searchResultValue.start())
    # start(): 匹配开始位置

    print(searchResultValue.end())
    # end(): 匹配结束位置

    print(searchResultValue.span())
    # span(): 返回 (start, end)

findallResultValue = patternObject.findall(sourceTextValue)
# findallResultValue: ['101', '202', '303']
print(findallResultValue)

finditerResultValue = patternObject.finditer(sourceTextValue)
# finditerResultValue: iterator[MatchObject]
for matchObjectValue in finditerResultValue:
    print(matchObjectValue.group(0), matchObjectValue.span())

splitResultValue = re.split(r",\s*", sourceTextValue)
# splitResultValue: 按逗号和可选空格进行切分
print(splitResultValue)

subResultValue = re.sub(r"Order-(\d+)", r"Ticket-\1", sourceTextValue)
# subResultValue: 用 Ticket-xxx 替换 Order-xxx
print(subResultValue)
```
...
![img](C:\Users\Administrator\Downloads\assets\Django-MVT-pattern.webp)
...
![img](C:\Users\Administrator\Downloads\assets\mermaid_20250509_bef6e2-scaled.png)
...
Django 对各种数据库提供了很好的支持，包括：PostgreSQL、MySQL、SQLite、Oracle。Django 为这些数据库提供了统一的调用API。我们可以根据自己业务需求选择不同的数据库。下文以 MySql 为例。
...
![img](C:\Users\Administrator\Downloads\assets\django-orm1.png)
...
![img](C:\Users\Administrator\Downloads\assets\orm-object.png)

###### 迁移流程

Django 使用迁移系统跟踪模型变更，并将模型定义同步到数据库中。常见命令如下：
...
虽然 Django 提供了多种操作方法，但是还是推荐使用 ORM 交互。
...
接收请求数据时，可以通过 `request.GET` 或 `request.POST` 访问表单提交的数据，并根据字段是否存在、字段值是否为空来决定后续处理逻辑。上面的 `search()` 示例中，就通过 `request.GET['q']` 读取查询参数，并根据内容是否为空返回不同响应。
...
Django 项目里多个 app 目录共用一个 urls 容易造成混淆，后期维护也不方便。使用路由分发（include），让每个 app 目录都单独拥有自己的 urls。
...
视图通常写在 `views.py` 中，并与路由建立映射关系。例如：
...
| POST          | 包含所有 HTTP POST 参数的类字典对象。服务器收到空的 POST 请求的情况也是有可能发生的。也就是说，表单通过 HTTP POST 方法提交请求，但是表单中可以没有数据。因此，不能使用语句 `if request.POST` 来判断是否使用 HTTP POST 方法；应该使用 `if request.method == "POST"`。注意：POST 不包括 file-upload 信息。参见 FILES 属性。 |
| REQUEST       | 为了方便，该属性是 POST 和 GET 属性的集合体，但是有特殊性，先查找 POST 属性，然后再查找 GET 属性。借鉴 PHP 的 `$_REQUEST`。例如，如果 `GET = {"name": "john"}` 和 `POST = {"age": '34'}`，则 `REQUEST["name"]` 的值是 `"john"`，`REQUEST["age"]` 的值是 `"34"`。强烈建议使用 GET 和 POST，因为这两个属性更加显式化，写出的代码也更易理解。 |
| COOKIES       | 包含所有 cookies 的标准 Python 字典对象。Keys 和 values 都是字符串。 |
| FILES         | 包含所有上传文件的类字典对象。FILES 中的每个 key 都是 `<input type="file" name="" />` 标签中 `name` 属性的值。FILES 中的每个 value 同时也是一个标准 Python 字典对象，包含下面三个 keys：`filename`、`content-type`、`content`。注意：只有在请求方法是 POST，并且请求页面中 `<form>` 有 `enctype="multipart/form-data"` 属性时 FILES 才拥有数据。否则，FILES 是一个空字典。 |
| META          | 包含所有可用 HTTP 头部信息的字典。例如：`CONTENT_LENGTH`、`CONTENT_TYPE`、`QUERY_STRING`、`REMOTE_ADDR`、`REMOTE_HOST`、`SERVER_NAME`、`SERVER_PORT` 等。META 中这些头加上前缀 `HTTP_` 为 key，冒号后面的为 value，例如：`HTTP_ACCEPT_ENCODING`、`HTTP_ACCEPT_LANGUAGE`、`HTTP_HOST`、`HTTP_REFERER`、`HTTP_USER_AGENT`。 |
| user          | 是一个 `django.contrib.auth.models.User` 对象，代表当前登录的用户。如果访问用户当前没有登录，user 将被初始化为 `django.contrib.auth.models.AnonymousUser` 的实例。你可以通过 user 的 `is_authenticated()` 方法来辨别用户是否登录。只有激活 Django 中的 AuthenticationMiddleware 时该属性才可用。 |
...
| 方法             | 描述                                                         |
| :--------------- | :----------------------------------------------------------- |
| __getitem__(key) | 返回 GET/POST 的键值，先取 POST，后取 GET。如果键不存在抛出 `KeyError`。也就是说，`request["foo"]` 等同于先 `request.POST["foo"]` 然后 `request.GET["foo"]`。 |
| has_key()        | 检查 `request.GET` 或 `request.POST` 中是否包含参数指定的 key。 |
...
| items()     | 和标准字典的 `items()` 方法有一点不同，该方法使用单值逻辑的 `__getitem__()`。 |
| values()    | 和标准字典的 `values()` 方法有一点不同，该方法使用单值逻辑的 `__getitem__()`。 |

此外，QueryDict 也有一些方法，如下表：

| **方法**                 | **描述**                                                     |
| :----------------------- | :----------------------------------------------------------- |
| copy()                   | 返回对象的拷贝，内部实现是用 Python 标准库的 `copy.deepcopy()`。该拷贝是 mutable（可更改的）。 |
| getlist(key)             | 返回和参数 key 对应的所有值，作为一个 Python list 返回。如果 key 不存在，则返回空 list。 |
| setlist(key, list_)      | 设置 key 的值为 list_（不同于 `__setitem__()`）。            |
| appendlist(key, item)    | 添加 item 到和 key 关联的内部 list。                         |
| setlistdefault(key,list) | 和 `setdefault` 有一点不同，它接受 list 而不是单个 value 作为参数。 |
| lists()                  | 和 `items()` 有一点不同，它会返回 key 的所有值，作为一个 list。 |
...
* **safe**：将字符串标记为安全，不需要转义。要保证 views.py 传过来的数据绝对安全，才能用 safe。和后端 views.py 的 `mark_safe` 效果相同。Django 会自动对 views.py 传到 HTML 文件中的标签语法进行转义，令其语义失效。加 safe 过滤器是告诉 Django 该数据是安全的，不必对其进行转义，可以让该数据语义生效。
...
根据条件判断是否输出。`if/else` 支持嵌套。`{% if %}` 标签接受 `and`、`or` 或者 `not` 关键字来对多个变量做判断，或者对变量取反。

**for 标签**

`{% for %}` 允许我们在一个序列上迭代。与 Python 的 for 语句类似，循环语法是 `for X in Y`，Y 是要迭代的序列而 X 是在每一个特定的循环中使用的变量名称。每一次循环中，模板系统会渲染在 `{% for %}` 和 `{% endfor %}` 之间的所有内容。**遍历字典**：可以直接用字典 `.items` 方法，用变量的解包分别获取键和值。可选的 `{% empty %}` 从句：在循环为空的时候执行（即 `in` 后面的参数布尔值为 False）。
...
`{% include %}` 标签允许在模板中包含其它模板的内容。下面这个例子包含了 `nav.html` 模板：
...
![img](C:\Users\Administrator\Downloads\assets\A995340B-8F8C-4777-9B79-846B6A34508A.jpg)
...
![img](C:\Users\Administrator\Downloads\assets\cookie.png)

一个 Web 服务器可以分配一个唯一的 session 会话 ID 作为每个 Web 客户端的 cookie，对于客户端的后续请求可以使用接收到的 cookie 来识别。

在 Web 开发中，使用 session 来完成会话跟踪，session 底层依赖 Cookie 技术。

![img](C:\Users\Administrator\Downloads\assets\cookie-1773910347379-13.png)
...
![img](C:\Users\Administrator\Downloads\assets\5-21-1.jpg)
...
![img](C:\Users\Administrator\Downloads\assets\1_t9TAX89Y3rZUXth2Le07Xg.png)
...
当配置多个中间件时，会按照 MIDDLEWARE 中的注册顺序执行 `process_request`，并在视图返回后按倒序执行 `process_response`。如果中间件某一步提前返回 `HttpResponse`，则后续执行流程会被截断，并直接进入响应阶段。
...
当配置多个中间件时，会按照 MIDDLEWARE 中的注册顺序，也就是列表的索引值，顺序执行。
...
process_response 方法有两个参数，一个是 request，一个是 response，request 是请求对象，response 是视图函数返回的 HttpResponse 对象，该方法必须要有返回值，且必须是 response。
...
![img](C:\Users\Administrator\Downloads\assets\md-sssss-1.png)
...
class MD1(MiddlewareMixin):
    ...
    def process_exception(self, request, exception):  # 引发错误才会触发这个方法
...
FastAPI 是一个用于构建 API 的现代、快速（高性能）的 web 框架，专为在 Python 中构建 RESTful API 而设计。FastAPI 使用 Python 3.8+ 并基于标准的 Python 类型提示。FastAPI 建立在 Starlette 和 Pydantic 之上，利用类型提示进行数据处理，并自动生成 API 文档。FastAPI 于 2018 年 12 月 5 日发布第一版本，以其易用性、速度和稳健性在开发者中间迅速流行起来。FastAPI 支持异步编程，可在生产环境中运行。
...
FastAPI 是一个用于构建 API 的现代、快速（高性能）的 web 框架，专为在 Python 中构建 RESTful API 而设计。FastAPI 使用 Python 3.8+ 并基于标准的 Python 类型提示。FastAPI 建立在 Starlette 和 Pydantic 之上，利用类型提示进行数据处理，并自动生成 API 文档。FastAPI 支持异步编程，可在生产环境中运行。
...
![img](C:\Users\Administrator\Downloads\assets\index-01-swagger-ui-simple.png)
...
在 FastAPI 中，路径参数和查询参数都可以通过函数参数直接声明，并结合类型提示进行自动解析和验证。
...
- `item_id` 是路径参数，来自 URL 路径 `/items/{item_id}`。
- `q` 是查询参数，例如 `/items/3?q=phone`。
- FastAPI 会根据类型提示自动将 `item_id` 转换为 `int`，并将 `q` 解析为可选字符串。
...
以上代码中，我们定义了一个名为 `Item` 的 Pydantic 模型，包含了四个字段：`name`、`description`、`price` 和 `tax`，`name` 和 `price` 是必需的字段，而 `description` 和 `tax` 是可选的字段，其默认值为 `None`。
...
以上代码中，`create_item` 路由处理函数接受一个名为 `item` 的参数，其类型是 `Item` 模型。FastAPI 将自动验证传入的 JSON 数据是否符合模型的定义，并将其转换为 `Item` 类型的实例。
...
![img](C:\Users\Administrator\Downloads\assets\fa461333139a7714dc4533af11d98dfe.png)
...
![img](C:\Users\Administrator\Downloads\assets\2ad6a4628a07ffede6521119828c083f.png)

返回结果如下：

![img](C:\Users\Administrator\Downloads\assets\e8cd27575ac43d40eb06c8fc4c6ff938.png)

##### 依赖项

FastAPI 提供了简单易用，但功能强大的依赖注入系统，这个依赖系统设计得简单易用，可以让开发人员轻松地把组件集成至 FastAPI。FastAPI 提供了路径操作依赖项（Path Operation Dependencies）的机制，允许你在路由处理函数执行之前或之后运行一些额外的逻辑。依赖项就是一个函数，且可以使用与路径操作函数相同的参数。路径操作依赖项提供了一种灵活的方式来组织代码、验证输入、进行身份验证等。
...
以下例子中，**common_parameters** 和 **verify_token** 是两个不同的依赖项函数，`verify_token` 依赖于 `common_parameters`，这种组合依赖项的方式允许我们在路由执行前先验证一些参数，然后再进行身份验证。
...
接下来我们设计一个接收一个登录表单数据的接口。要使用表单，需预先安装 `python-multipart`：
...
![img](C:\Users\Administrator\Downloads\assets\927e7dfc95fb795eeaa3af240662cb94.png)
...
![img](C:\Users\Administrator\Downloads\assets\514028636203aef522a8addb3f4eb62a.png)
...
![img](C:\Users\Administrator\Downloads\assets\0d28c66e90a6ad39fc4ef693c146835c.png)

![img](C:\Users\Administrator\Downloads\assets\7eb01111becfc49e6e380300ad66c7a3.png)
...
![img](C:\Users\Administrator\Downloads\assets\index-01-swagger-ui-simple-1773918165488-17.png)
...
![img](C:\Users\Administrator\Downloads\assets\index-02-redoc-simple.png)
...
`asyncio` 可以实现单线程并发 I/O 操作。如果仅用在客户端，发挥的威力不大。如果把 `asyncio` 用在服务器端，例如 Web 服务器，由于 HTTP 连接就是 I/O 操作，因此可以用单线程 + `async` 函数实现多用户的高并发支持。
...
下面编写一个 HTTP 服务器，分别处理以下 URL：

- `/`：首页返回 `Index Page`
- `/{name}`：根据 URL 参数返回文本 `Hello, {name}!`
...
![Index](C:/Users/Administrator/Downloads/assets/index.png)

访问 `http://localhost:8080/Bob`：

![Hello](C:/Users/Administrator/Downloads/assets/hello.png)

使用 aiohttp 时，通常先定义处理不同 URL 的 `async` 函数，然后通过 `app.add_routes()` 添加路由映射，最后通过 `web.run_app()` 启动应用。



### 数据可视化

Dash 是一个基于 Python 的开源框架，专门用于构建数据分析和数据可视化的 Web 应用程序。Dash 由 Plotly 团队开发，旨在帮助数据分析师、数据科学家和开发人员快速创建交互式的、基于数据的 Web 应用，而无需深入掌握前端技术（如 HTML、CSS 和 JavaScript）。Dash 的核心优势在于其简单易用性和强大的功能。通过 Dash，用户可以使用纯 Python 代码来构建复杂的 Web 应用，而无需编写繁琐的前端代码。Dash 应用通常由两个主要部分组成：**布局**和**交互性**。Dash 结合了 Flask 的后端能力、Plotly.js 的可视化能力以及 React.js 的交互能力，为用户提供了一个简单而强大的开发平台。
...
Dash 允许开发者使用 Python 来创建动态的、响应式的用户界面。Dash 的核心功能之一就是回调函数（Callback），它使得用户界面能够根据用户的输入或操作实时更新。回调函数是 Dash 中用于处理用户交互的核心机制，它允许你在用户与应用程序交互时，动态地更新应用程序的布局或数据。简单来说，回调函数是一个 Python 函数，它会在特定的输入发生变化时被触发，并根据这些输入的变化来更新输出。
...
在开发交互式 Web 应用程序时，Dash 不仅允许你使用 Python 来构建复杂的应用程序，还提供了丰富的样式设计选项，使你的应用程序看起来更加专业和美观。在 Dash 中，样式设计是通过 CSS 实现的。

Dash 提供了多种方式来设置组件的样式，包括：
...
操作系统在任何给定的时间内，只有一个 greenlet 在运行。

这不同于由 `multiprocessing` 或 `threading` 库提供的并行结构，这些库可以创建进程和 POSIX 线程，由操作系统调度，并且是真正并行的。
...
当我们将 gevent 用于网络和 I/O 绑定函数时，它的真正威力就来了，这些函数可以协同调度。gevent 负责处理所有细节，以确保网络库尽可能隐式地让步出它们的 greenlet 上下文。
...
def gr1():
    ...
    # 可以理解成一个 I/O 阻塞的操作，阻塞 2 秒，这时 Greenlet 会自动切换到 gr2() 上下文执行
    select.select([], [], [], 2)
...
def gr3():
    ...
    gevent.sleep(1)
    # 让当前 Greenlet 休眠 1 秒，上面 gr1() gr2() 阻塞操作完成后，再切换到 gr1() gr2() 的上下文执行
...
程序的重要部分是 `gevent.spawn`，它将给定的函数封装在一个 Greenlet 线程中。初始化的 greenlet 列表存储在传递给 `gevent.joinall` 的数组中。`gevent.joinall` 会阻塞当前程序，来运行所有给定的 greenlet。只有当所有 greenlet 终止时，执行才会继续进行。

需要注意的是，异步情况下的执行顺序本质上是随机的，异步情况下的总执行时间比同步情况下少得多。实际上，同步的例子完成的最大时间是每个任务停顿 0.002 秒，导致整个队列停顿 0.02 秒。在异步情况下，最大运行时间大约为 0.002 秒，因为没有一个任务会阻塞其他任务的执行。

在更常见的用例中，异步地从服务器获取数据，`fetch()` 的运行时在请求之间会有所不同，这取决于请求时远程服务器上的负载。

```python
import gevent.monkey
gevent.monkey.patch_socket()  # 把内置的阻塞的 socket 替换成非阻塞的 socket
...
尽管 gevent 通常是确定的，但当开始与外部服务（如 socket 和文件）进行交互时，非确定性的来源可能会潜入程序。因此，即使 greenlet 是确定性并发的一种形式，它们仍然会遇到 POSIX 线程和进程遇到的一些相同的问题。

与并发有关的长期问题称为竞争条件。简而言之，当两个并发线程 / 进程依赖于某些共享资源但还试图修改该值时，就会发生竞争状态。这将导致资源的值变得依赖于执行顺序。这是一个问题，一般来说，应该尽量避免竞态条件，因为它们会导致全局的不确定程序行为。

最好的方法是在任何时候都避免所有全局状态。全局状态和导入时间的副作用总是会回来“咬你一口”。
...
# Wrapper for creating and running a new Greenlet from the named function foo
...
除了使用基本的 Greenlet 类，您还可以子类化 `Greenlet` 类并覆盖 `_run` 方法。
...
与代码的其他部分一样，greenlet 可能以各种方式失败。greenlet 可能抛出异常、无法停止或消耗太多系统资源。

greenlet 的内部状态通常是一个与时间相关的参数。在 greenlet 上有许多标志，它们允许您监视线程的状态：

- `started` — 布尔值，指示 Greenlet 是否已启动
- `ready()` — 布尔值，指示 Greenlet 是否已停止
- `successful()` — 布尔值，指示 Greenlet 是否已停止且没有抛出异常
- `value` — Greenlet 返回的值
- `exception` — 在 greenlet 中抛出的未捕获异常实例
...
# The exception raised in fail, will not propagate outside the greenlet.
...
当主程序接收到 `SIGQUIT` 时，无法生成（yield）的 greenlet 可能会使程序的执行时间比预期更长。这将导致所谓的“僵尸进程”，需要从 Python 解释器外部杀死这些进程。
...
我们来到了 gevent 的黑暗角落。到目前为止，我一直避免提到 monkey patching，以尝试激发强大的协同模式，但是现在是讨论 monkey patching 的黑魔法的时候了。如果注意到上面我们调用了命令 `monkey.patch_socket()`，这是一个纯粹用于修改标准库套接字库（socket）的副作用命令。
...
Python 允许在运行时修改大多数对象，包括模块、类甚至函数。这通常是一个令人震惊的坏主意，因为它创建了一个“隐式副作用”，如果出现问题，通常非常难以调试。然而在极端情况下，库需要改变 Python 本身的基本行为，可以使用 monkey 补丁。

在这种情况下，gevent 能够修补标准库中的大多数阻塞系统调用，包括 `socket`、`ssl`、`threading` 和 `select` 模块中的调用。
...
队列是按顺序排列的数据集，它们具有通常的 `put / get` 操作，但可以以在 Greenlet 上安全操作的方式编写。

例如，如果一个 Greenlet 从队列中获取一个项目（item），则同一项目不会被同时执行的另一个 Greenlet 获取。
...
每个 `put` 和 `get` 操作都有一个非阻塞的对应操作，`put_nowait` 和 `get_nowait` 不会阻塞。如果操作不可能，会引发 `gevent.queue.Empty` 或 `gevent.queue.Full`。

在这个例子中，我们让 boss 同时和 workers 运行，并且对队列有一个限制，防止它包含三个以上的元素。这个限制意味着 `put` 操作将阻塞，直到队列上有空间为止。相反，如果队列上没有要获取的元素，`get` 操作就会阻塞，它还会接受一个超时参数，如果在超时的时间范围内找不到工作，则允许队列以异常 `gevent.queue.Empty` 退出。
...
池是一种结构，用于处理需要限制并发的动态数量的 greenlet。在需要并行执行许多网络或 I/O 绑定任务的情况下，这通常是可取的。
...
信号量是一种低级同步原语，它允许 greenlet 协调和限制并发访问或执行。信号量公开两种方法，获取和释放。信号量被获取和释放的次数之差称为信号量的界限。如果信号量范围达到 0，它就会阻塞，直到另一个 greenlet 释放它的获取。
...
界限为 1 的信号量称为锁。它提供对一个 greenlet 的独占执行。它们通常用于确保资源只在程序上下文中使用一次。
