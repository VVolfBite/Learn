...
特性（Attribute）与反射（Reflection）本质上是在“类型系统之上再加一层可查询的描述信息”。面向对象里，类型不仅是代码结构，更是运行时可以被检查与操作的对象：程序集 → 类型 → 成员 → 元数据。特性用于把声明性信息贴在类/方法/属性上（例如标记过时、序列化规则、路由、依赖注入等），反射用于在运行时读取这些信息并据此做行为（扫描、注册、映射、自动化）。所以它们经常成对出现：**Attribute 负责描述，Reflection 负责读取与执行策略**。熟练级别只需会：定义特性、用 AttributeUsage 限制目标、用反射读取并做一次简单决策。写特性时，如果特性类型名以 `Attribute` 结尾，那么在 `[...]` 里可以省略这个后缀——例如特性类型叫 `StudentCommandAttribute`，使用时既可以写 `[StudentCommandAttribute("add-student")]`，也可以简写成 `[StudentCommand("add-student")]`；这个省略只在特性标注语法中成立，并不代表你平时也有一个叫 `StudentCommand` 的类型。其次，`[...]` 里括号的参数来自特性构造函数：当特性类定义了 `StudentCommandAttribute(string name)` 这样的构造函数时，`[StudentCommand("add-student")]` 的 `"add-student"` 就是传给该构造函数的参数，编译后会作为元数据写进程序集；运行时通过反射读取该特性时（例如 `GetCustomAttribute<StudentCommandAttribute>()`），运行库会依据元数据创建特性实例，因此你能在代码里取到 `Attr.Name == "add-student"`。另外还存在“命名参数”（named arguments）这种写法，用来给特性的**可写属性**或**公共字段**赋值（例如 `Level = 2`）；但如果某个属性只有 getter 没有 setter（像这里的 `Name`），那它就不能用命名参数赋值，只能通过构造函数参数传入。

**基础用法列举**

| 目的                  | 特性写在哪里       | 反射怎么读                       |
| --------------------- | ------------------ | -------------------------------- |
| 标注“这是命令/处理器” | class 上           | `type.GetCustomAttributes(...)`  |
| 标注“字段映射名”      | property 上        | `prop.GetCustomAttributes(...)`  |
| 约束特性可用位置      | `AttributeUsage`   | 编译期帮助纠错                   |
| 运行时扫描注册        | assembly/type 扫描 | `Assembly.GetTypes()`            |
...
namespace StudentManagerApp
{
    ...
    public sealed class StudentCommandAttribute : Attribute
    {
        public string Name { get; }
        public StudentCommandAttribute(string name) => Name = name;
    }

    [StudentCommand("add-student")]
    ...
    public static class CommandScanner
    {
        public static void Scan()
        {
            ...
            var handlers = asm.GetTypes()
                .Select(t => new { Type = t, Attr = t.GetCustomAttribute<StudentCommandAttribute>() })
                .Where(x => x.Attr is not null);

            foreach (var h in handlers)
                Console.WriteLine($"Command: {h.Attr!.Name} -> {h.Type.Name}");
        }
    }
}
```
...
索引器（Indexer）可以把一个类做成“像容器一样能用 `[]` 访问”的样子。你平时写 `arr[0]`、`dict["key"]`，其实是在“给某个访问入口传一个索引/键，然后取回或写入一个值”。索引器就是把这种入口开放给你自己的类型：你定义好“收到一个 key（比如科目名）后该怎么取值/写值”，外部就可以直接写 `obj[key]`。因此它很适合表达“仓储/表/映射/配置”这类概念：读起来像从一个容器里按 key 拿东西，而不是在调用一个动作型方法。

从语言结构上看，索引器和属性很像：它同样是 `get`/`set`，也同样可以只读或只写；只是它多了“参数”，所以访问时用 `[]` 而不是点号。你可以把它当成“带参数的属性”来记：普通属性是 `obj.Age`，索引器是 `obj[key]`。语法上，索引器必须写成 `this[...]`，这里的 `this` 不是变量名、也不是你能改的名字，而是 C# 规定的索引器声明形式：意思是“这个类的实例可以用 `[]` 接收这些参数”。如果你想提供多个 `[]` 入口，做法不是给索引器改名，而是换参数签名：比如同一个类里可以同时存在 `this[int id]` 和 `this[string subject]`，编译器会根据你传的参数类型自动选中合适的那个索引器。

实际工程里最容易踩的点是：索引器只是入口，它不会替你表达“查不到怎么办”。也就是说 `obj[key]` 这句语法本身不携带“成功/失败”的状态，你必须自己约定 key 不存在时的行为：是抛异常、返回 `null`、还是返回默认值。一般来说，如果这段访问代表“按业务逻辑这里必须有值”（例如内部流程里读一个已经录入过的成绩），抛异常反而更清晰，因为它能尽早暴露错误；而如果 key 来自用户输入或外部数据（可能拼错、可能没录入），那就不适合让 `[]` 频繁抛异常，这时通常会搭配一个 `TryGet...` 风格的方法，让调用方通过布尔值判断是否存在，再决定下一步怎么处理。你也可以把 `[]` 理解成“强语义入口”，把 `TryGet...` 理解成“安全入口”，两者并存可读性更好。

另外，索引器内部到底怎么实现，完全由你决定：很多例子会用 `Dictionary`，因为它天然就是 key→value；但索引器并不要求内部必须是字典。你可以在 `get/set` 里访问字段/属性、做计算、读取配置、访问数据库……索引器只是把这些逻辑包装成一个统一的 `[]` 接口而已。

下面我们把“一个学生有多门课成绩”建模成：学生内部维护一张“科目名 → 分数”的表。这样我们就能自然写出：

- `student["Math"] = 98;`（录入/更新成绩）
- `var score = student["Math"];`（读取成绩）
- `student.TryGetScore("Biology", out var s)`（安全读取：可能不存在）

> 约定：`[]` 的 `get` 在科目不存在时抛异常（强语义），同时提供 `TryGetScore` 给外部输入使用。
...
namespace StudentManagerApp
{
    public class Student
    {
        ...
        // Real storage: subject -> score (case-insensitive keys)
        ...
        // Indexer: enables student["Math"] / student["English"]
        // Missing key => throw (strong semantics)
        public int this[string subject]
        {
            get
            {
                ...
                if (!_scores.TryGetValue(subject, out var score))
                    throw new KeyNotFoundException($"Score not found: {subject}");
            ...
            set
            {
                ...
                // Write/overwrite: add if missing, overwrite if exists
                _scores[subject] = value;
            }
        }

        // Safe read path: good for user input / uncertain keys
        public bool TryGetScore(string subject, out int score)
        {
            if (subject is null) { score = default; return false; }
            return _scores.TryGetValue(subject, out score);
        }

        // Add-only semantics (optional): reject duplicates explicitly
        public void AddScore(string subject, int score)
        {
            ...
            if (_scores.ContainsKey(subject))
                throw new InvalidOperationException($"Duplicate subject: {subject}");

            _scores.Add(subject, score);
        }
    }
    ...
    public static class Program
    {
        public static void Main()
        {
            ...
            // 3) 安全读取（你不确定是否存在时用）
            if (student.TryGetScore("Biology", out var bio))
                Console.WriteLine($"Biology: {bio}");
            else
                Console.WriteLine("Biology: (no score)");

            // 4) 演示：不存在时索引器 get 会抛异常
            try
            {
            ...
            {
                Console.WriteLine($"History read failed: {ex.Message}");
            }

            // 5) 演示：AddScore 语义更强（重复科目会拒绝）
            student.AddScore("Physics", 88);
            try
            {
            ...
            {
                Console.WriteLine($"AddScore failed: {ex.Message}");
            }
        }
    }
}
...
#### 析构函数（Finalizer）与 IDisposable：资源释放在 C# 的正确姿势
...
委托（delegate）最直观的作用是“把一段可执行的逻辑当成数据传来传去”，也就是你可以把方法塞进变量、作为参数传递、放进集合里；事件（event）则是在委托之上再加了一层“发布-订阅的约束”，把“谁有权触发通知”这件事固定下来。用 OOP 的眼光看，发布者对象拥有状态与变化点，它的职责是：状态发生变化时把变化广播出去；订阅者对象只负责：订阅、响应、在不需要时退订。订阅者不应该能直接触发发布者的通知，更不应该能把整个委托链替换掉，这就是 `event` 和裸委托最大的差别：`event` 把“触发权”留在发布者内部，外部只能 `+=`/`-=`。

工程里最常见的一套写法是 `EventHandler` / `EventHandler<TEventArgs>`，因为它把“谁触发（sender）”与“触发时携带的上下文（args）”分开，读代码时一眼就能看出这是标准事件。触发通常会被封装到一个 `protected virtual OnXxx(...)` 方法里，一方面让发布者内部调用点统一（避免到处散落 `?.Invoke`），另一方面也给派生类留出扩展空间：子类可以在 OnXxx 前后加额外动作，但不必破坏订阅者侧的调用方式。

| 你在代码里会看到的点                 | 它在解决什么问题          | 读到它时的直觉               |
| ------------------------------------ | ------------------------- | ---------------------------- |
| `public event EventHandler<T>? Xxx;` | 建立通知通道              | 外部只能订阅/退订，不能触发  |
| `Xxx?.Invoke(this, args);`           | 触发事件（发布者内部）    | 这里是“状态变化点”           |
| `protected virtual void OnXxx(...)`  | 统一触发入口 + 给子类扩展 | 派生类能加行为但不改契约     |
| 订阅 `+=` / 退订 `-=`                | 生命周期管理              | 谁订阅谁负责退订（避免泄漏） |

下面的示例沿用 StudentManagerApp：StudentManager 是发布者，当“学生被添加/移除/更新”时发出通知；控制台订阅者是一个最简单的 Subscriber；再补一个“统计订阅者”，让你看到同一个事件可以被多个对象订阅。Main 里会演示订阅与退订的效果。
...
namespace StudentManagerApp
{
    public sealed class Student
    {
        ...
        public void Rename(string name) => Name = string.IsNullOrWhiteSpace(name) ? Name : name.Trim();
        public override string ToString() => $"#{Id} {Name}";
    }
    ...
    public sealed class StudentChangedEventArgs : EventArgs
    {
        ...
        public StudentChangedEventArgs(int id, string action)
        {
            StudentId = id;
            Action = action;
        }
    ...
    {
        private readonly Dictionary<int, Student> _map = new();

        public event EventHandler<StudentChangedEventArgs>? StudentChanged;

        protected virtual void OnStudentChanged(int id, string action)
            => StudentChanged?.Invoke(this, new StudentChangedEventArgs(id, action));

        public void Add(Student s)
        {
            _map[s.Id] = s;
            OnStudentChanged(s.Id, "Add");
        }

        public bool Remove(int id)
        {
            if (!_map.Remove(id)) return false;
            OnStudentChanged(id, "Remove");
            return true;
        }

        public bool Rename(int id, string newName)
        {
            if (!_map.TryGetValue(id, out var s)) return false;
            s.Rename(newName);
            OnStudentChanged(id, "Rename");
            return true;
        }
    }

    public sealed class ConsoleSubscriber
    {
        public void Subscribe(StudentManager mgr) => mgr.StudentChanged += Handle;
        public void Unsubscribe(StudentManager mgr) => mgr.StudentChanged -= Handle;

        private void Handle(object? sender, StudentChangedEventArgs e)
            => Console.WriteLine($"[Console] {e.Action} student #{e.StudentId}");
    }

    public sealed class CounterSubscriber
    {
        public int Count { get; private set; }
        public void Subscribe(StudentManager mgr) => mgr.StudentChanged += Handle;
        public void Unsubscribe(StudentManager mgr) => mgr.StudentChanged -= Handle;

        private void Handle(object? sender, StudentChangedEventArgs e) => Count++;
    }
    ...
    public static class Program
    {
        public static void Main()
        {
            ...
            var console = new ConsoleSubscriber();
            var counter = new CounterSubscriber();

            console.Subscribe(mgr);
            counter.Subscribe(mgr);

            mgr.Add(new Student(1, "Alice"));
            mgr.Rename(1, "Alice Zhang");
            mgr.Remove(1);

            Console.WriteLine($"Counter = {counter.Count}");

            console.Unsubscribe(mgr); // 演示退订
            mgr.Add(new Student(2, "Bob")); // 这次控制台不会再输出，但 Counter 仍会计数
...
运算符重载的价值在于“把类型语义写得更接近自然阅读”，比如成绩区间、分数段、学号范围这类值对象，本身就有清晰的比较与相等语义，用 `==`、`<`、`Contains` 会比一堆 `Compare(x,y)` 更易读。麻烦也来自这里：一旦你重载了 `==/!=`，读者会自然地把它理解为“按值相等”，而集合（Dictionary/HashSet）又非常依赖 `Equals/GetHashCode` 来判断相等，所以工程里最重要的原则是：如果你让 `==` 表达按值相等，那就必须让 `Equals/GetHashCode` 用同一套字段集合表达同样的相等性，否则会出现“看起来相等但 HashSet 不去重”这种非常难查的 bug。

排序这件事又分成两层：类型自己“默认/自然”的排序（例如 ScoreBand 按 Min 再按 Max）适合放在 `IComparable<T>`；不同场景下的排序规则（按姓名、按年龄、按成绩）更适合放在 `IComparer<T>` 或 `Comparison<T>`，因为它不污染类型本身的默认语义，也更利于调用方按场景选择。

| 你要表达的语义             | 更推荐的做法                               | 你会在调用处看到什么                   | 重点风险                 |
| -------------------------- | ------------------------------------------ | -------------------------------------- | ------------------------ |
| “这个值对象能不能比较大小” | `IComparable<T>`                           | `list.Sort()` 或 `Comparer<T>.Default` | 比较逻辑要稳定一致       |
| “不同场景要不同排序”       | `IComparer<T>` / `Comparison<T>`           | `list.Sort(new XxxComparer())`         | 不要把场景规则硬写进类型 |
| “按值相等”                 | `record` 或 `Equals/GetHashCode` + `==/!=` | `a == b`、HashSet 去重                 | 三者必须一致             |

示例继续用学生系统：ScoreBand 是值对象，天然比较与按值相等；Student 的“自然排序”通常不建议硬塞进 Student 类型（因为不同页面可能按不同字段排），所以演示用一个 `StudentNameComparer` 来做场景化排序。Main 里会演示 Sort、HashSet 去重，以及 `==` 的一致性。
...
namespace StudentManagerApp
{
    public readonly struct ScoreBand : IComparable<ScoreBand>, IEquatable<ScoreBand>
    {
        public int Min { get; }
        public int Max { get; }

        public ScoreBand(int min, int max)
        {
            if (min > max) throw new ArgumentException("min must <= max");
            Min = min; Max = max;
        }

        public int CompareTo(ScoreBand other)
        {
            int c = Min.CompareTo(other.Min);
            return c != 0 ? c : Max.CompareTo(other.Max);
        }

        public bool Equals(ScoreBand other) => Min == other.Min && Max == other.Max;
        public override bool Equals(object? obj) => obj is ScoreBand sb && Equals(sb);
        public override int GetHashCode() => HashCode.Combine(Min, Max);

        public static bool operator ==(ScoreBand a, ScoreBand b) => a.Equals(b);
        public static bool operator !=(ScoreBand a, ScoreBand b) => !a.Equals(b);
    ...
    public sealed class Student
    {
        public int Id { get; }
        public string Name { get; }
        public Student(int id, string name) { Id = id; Name = name.Trim(); }
        public override string ToString() => $"#{Id} {Name}";
    }

    public sealed class StudentNameComparer : IComparer<Student>
    {
    ...
    public static class Program
    {
        public static void Main()
        {
            var bands = new List<ScoreBand>
            {
                new ScoreBand(60, 69),
                new ScoreBand(90, 100),
                new ScoreBand(70, 79),
                new ScoreBand(60, 69),
            };
            bands.Sort();
            Console.WriteLine("== Sorted ScoreBand ==");
            foreach (var b in bands) Console.WriteLine(b);

            var set = new HashSet<ScoreBand>(bands);
            Console.WriteLine($"HashSet count = {set.Count} (should dedup equal bands)");
            ...
            Console.WriteLine("== Students sorted by name (case-insensitive) ==");
...
读现代 C# 工程代码时，经常会看到三类“让表达更直接”的语法：模式匹配、`required`、扩展方法。模式匹配把“按类型/按条件分支”写得更贴合多态读法：你不用先 `as` 再判空，也不用一堆强转；`required` 则是把“对象必须填哪些字段”交给编译器检查，尤其适合 DTO/命令对象这类“输入边界数据”，避免漏填；扩展方法的价值在于把“工具逻辑”从实体类里拿出来，但调用时仍像实例方法一样自然，既不污染实体，也能保持可读性。

| 语法       | 它更像在补哪块能力 | 你会在代码里看到的典型写法                   | 读到它的直觉                 |
| ---------- | ------------------ | -------------------------------------------- | ---------------------------- |
| 模式匹配   | 更安全的下转与分支 | `if (p is Student s)` / `obj switch { ... }` | 这是“按真实类型处理”的分支   |
| `required` | 输入对象必须完整   | `public required string Name { get; init; }` | 这是“缺字段就编译不过”的约束 |
| 扩展方法   | 工具能力外置       | `students.ToPrettyList()`                    | 这是“工具类伪装成实例方法”   |

示例仍然围绕 StudentManagerApp：CreateStudentCommand 是典型边界输入；扩展方法负责格式化输出；模式匹配负责把 object/Person 的分支处理写清楚。Main 里会展示 required 的初始化方式、扩展方法的调用方式，以及 switch 模式匹配的结果。
...
namespace StudentManagerApp
{
    public sealed class Student
    {
        ...
        public string Name { get; }
        public int? Age { get; }
        public Student(int id, string name, int? age = null)
        {
            Id = id;
            Name = name.Trim();
            Age = age;
        }
        public override string ToString() => $"#{Id} {Name}" + (Age is null ? "" : $" ({Age})");
    }
    ...
    {
        public static string ToPrettyText(this Student s) => s.ToString();

        public static string ToPrettyList(this IEnumerable<Student> students)
        {
            var sb = new StringBuilder();
            foreach (var s in students) sb.AppendLine(s.ToPrettyText());
            return sb.ToString();
        }
    }

    public static class PolymorphismPatterns
    {
        public static string DescribeObject(object? obj) =>
            obj switch
            {
                null => "<null>",
                Student s => $"Student => {s.ToPrettyText()}",
    ...
    public static class Program
    {
        public static void Main()
        {
            var cmd = new CreateStudentCommand { Id = 1, Name = "Alice", Age = 20 };
            var student = new Student(cmd.Id, cmd.Name, cmd.Age);

            var list = new List<Student> { student, new Student(2, "Bob") };
            Console.WriteLine("== Pretty List ==");
            Console.WriteLine(list.ToPrettyList());

            Console.WriteLine("== Pattern Match ==");
            Console.WriteLine(PolymorphismPatterns.DescribeObject(student));
            Console.WriteLine(PolymorphismPatterns.DescribeObject("just a string"));
            Console.WriteLine(PolymorphismPatterns.DescribeObject(null));
        }
    }
}
```
...
相等性在工程里不是“写不写都行”的修饰，它会直接影响集合行为。`List.Contains`、`HashSet` 去重、`Dictionary` 作为 key 的查找，底层都依赖 `Equals/GetHashCode` 的一致性。默认情况下，`class` 的相等性更偏引用语义（除非你重写），这对“实体”通常是合理的：两个 Student 是否同一人往往由 Id 决定，而不是所有字段都一样；值对象/DTO 则更适合按字段内容判断相等，这时 `record` 很省事，它默认就按字段集合生成 `Equals/GetHashCode`，可读性也更好。

| 你正在建模的东西            | 更像什么         | 推荐相等性策略                    | 在学生系统里常见例子 |
| --------------------------- | ---------------- | --------------------------------- | -------------------- |
| 学生（有唯一 Id）           | 实体（identity） | `Equals/GetHashCode` 基于 Id      | `Student`            |
| 成绩区间、分数段            | 值对象（value）  | `record` 或 IEquatable + 组合哈希 | `ScoreRange`         |
| 作为 HashSet/Dictionary key | 集合关键行为     | 必须保证一致性                    | “相等”就要“同 hash”  |

示例把 Student 的相等性明确为“Id 相同就认为同一个学生”，并用 Dictionary/HashSet 演示其影响；ScoreRange 用 record 直接获得按值相等，演示去重。
...
namespace StudentManagerApp
{
    ...
    public static class Program
    {
        ...
        {
            var s1 = new Student(1, "Alice");
            var s2 = new Student(1, "Alice (renamed)");
            Console.WriteLine($"s1.Equals(s2) = {s1.Equals(s2)} (by Id)");

            var set = new HashSet<Student> { s1, s2 };
            Console.WriteLine($"HashSet<Student> count = {set.Count} (should be 1)");

            var ranges = new HashSet<ScoreRange>
            {
                new ScoreRange(60, 69),
                new ScoreRange(60, 69),
                new ScoreRange(70, 79),
            };
            Console.WriteLine($"HashSet<ScoreRange> count = {ranges.Count} (record value equality)");
        }
    }
}
```
...
把接口/抽象类当作边界契约时，错误处理方式就变成“所有实现与调用方都会被迫遵守的交互协议”。工程上通常把失败分成两类：一种是“预期分支”，比如按 id 查学生可能找不到，这更像业务分支而不是程序错误；另一种是“违反契约或系统故障”，比如参数非法、数据损坏，这类更适合抛异常。Try 模式（返回 bool + out）就是把“预期分支”显式地写进 API 里，调用方读到方法名就知道要走分支，而不是被异常控制流打断。

| 失败类型   | 更像什么          | 推荐交互方式                | 学生系统里的典型例子 |
| ---------- | ----------------- | --------------------------- | -------------------- |
| 查找不存在 | 预期分支          | `TryGet...` / `Student?`    | `TryGetById`         |
| 参数不合法 | 违反契约          | 抛 `ArgumentException` 系列 | `Add(null)`、id < 0  |
| 系统故障   | 不可恢复/需要上报 | 抛异常 + 日志               | IO 失败、数据损坏    |

示例仍保持 StudentRepository 的语境，并加 Main 演示“查不到就是 false”“重复 id 是异常”的边界。
...
namespace StudentManagerApp
{
    ...
    {
        bool TryGetById(int id, out Student? student);
    ...
    public sealed class StudentRepository : IStudentRepository
    {
        ...
        public bool TryGetById(int id, out Student? student) => _map.TryGetValue(id, out student);

        public void Add(Student student)
        {
            ArgumentNullException.ThrowIfNull(student);
            if (_map.ContainsKey(student.Id))
                throw new InvalidOperationException("Duplicate student id.");
            _map[student.Id] = student;
        }
    }
    ...
    public static class Program
    {
        public static void Main()
        {
            ...
            repo.Add(new Student(1, "Alice"));

            if (repo.TryGetById(2, out var s))
                Console.WriteLine(s);
            else
                Console.WriteLine("Student #2 not found (expected branch)");

            try
            {
                repo.Add(new Student(1, "Alice Duplicate"));
            }
            catch (InvalidOperationException ex)
            {
...
广义上把“一个名字对应多种行为”都叫多态没问题，但读 C# 工程代码时，更有用的分法是：调用目标到底是在编译期就固定了，还是要等到运行时根据对象真实类型决定。方法重载、运算符重载属于编译期绑定，编译器会根据参数类型直接选定要调用的签名；`virtual/override` 与接口分派属于运行时绑定，你拿着的是基类/接口引用，最终调用哪个实现要等到运行时看对象到底是谁。这个边界会影响你理解扩展点、调试策略与性能预期。

| 机制             | 决策时机 | 你在代码里看到的典型形态    | 读到它时的直觉                 |
| ---------------- | -------- | --------------------------- | ------------------------------ |
| 方法重载         | 编译期   | 同名不同参数                | “编译器已经选定了目标”         |
| 运算符重载       | 编译期   | `operator + / ==`           | “语法糖，编译器绑定”           |
| virtual/override | 运行时   | 基类引用调用虚方法          | “真正的 OOP 多态扩展点”        |
| 接口分派         | 运行时   | `IStudentStore store = ...` | “实现可替换，调用按真实对象走” |

示例用同一个名字 `Describe` 演示编译期与运行时的差异：重载在编译期决定；override 在运行时决定。
...
namespace StudentManagerApp
{
    ...
    public static class Program
    {
        ...
        {
            var p = new Printer();
            p.Print(1);     // 编译期：选 Print(int)
            p.Print("x");   // 编译期：选 Print(string)

            Person who = new Student();
            Console.WriteLine(who.Describe()); // 运行时：走 Student override
        }
    }
}
```
...
多态的核心价值是“调用方依赖契约，实现可替换”。在 C# 里契约最常见的是接口：你可以写出只依赖 `IStudentStore` 的 StudentManager，然后在不同环境下换成内存实现、文件实现、网络实现。落地之后常见的需求是：你手里拿的是接口/基类引用，但你确实需要对某些具体类型做差异处理，这时推荐用 `is`/switch 模式匹配，既安全又清晰，尽量避免裸强转。与此同时，如果类型会进入集合、会被比较、会参与日志输出，你通常还会看到 `ToString/Equals/GetHashCode` 这套 Object 方法配套，它们并不是“装饰”，而是集合与调试的基础设施。

| 工具               | 解决的日常问题   | 常见写法                                          | 读到它时的直觉       |
| ------------------ | ---------------- | ------------------------------------------------- | -------------------- |
| 接口多态           | 可替换实现       | `IStudentStore store = new MemoryStudentStore();` | 边界清晰、好测试     |
| 模式匹配           | 安全下转分支     | `if (p is Student s) ...` / `switch`              | 只在需要差异处理时用 |
| ToString           | 调试与日志可读性 | `override string ToString()`                      | 输出更像业务语言     |
| Equals/GetHashCode | 集合行为一致     | 成对重写或 record                                 | key/去重行为靠它     |

示例把“存储”与“输出”都串起来：store 用接口替换；Describe 用模式匹配；Student 重写 ToString 便于日志。Main 演示同一段业务逻辑换 store 就能跑。
...
namespace StudentManagerApp
{
    ...
    public sealed class MemoryStudentStore : IStudentStore
    {
        private readonly List<Student> _cache = new();
        public void Save(List<Student> students)
        {
        ...
        public List<Student> Load() => new List<Student>(_cache);
    }
    ...
    public sealed class StudentManager
    {
        ...
        public string DescribeAny(object? obj) =>
            obj switch
            {
                Student s => $"Student => {s}",
                null => "<null>",
    ...
    public static class Program
    {
        public static void Main()
        {
            IStudentStore store = new MemoryStudentStore();
            var mgr = new StudentManager(store);

            mgr.SaveSome();
            Console.WriteLine("== Load & Print ==");
            mgr.PrintAll();

            Console.WriteLine("== DescribeAny ==");
            Console.WriteLine(mgr.DescribeAny(new Student(3, "Cathy")));
            Console.WriteLine(mgr.DescribeAny("hello"));
        }
    }
}
```
