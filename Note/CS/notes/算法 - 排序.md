...
排序是最基础也是最重要的算法之一。掌握各种排序算法不仅是为了应对面试，更重要的是理解它们背后的思想——这些思想在解决其他问题时同样适用。

在分析排序算法时，我们主要关注比较次数和交换次数，这是排序算法的成本模型。为了让代码更清晰，通常会定义辅助函数 less() 用于比较，swap() 用于交换。
...
选择排序的思路很直观：每次从未排序部分找出最小元素，放到已排序部分的末尾。具体来说，第一次从整个数组中找最小元素放到第一位，第二次从剩余元素中找最小元素放到第二位，以此类推。

选择排序需要约 N²/2 次比较和 N 次交换。它有个特点是运行时间与输入无关——即使数组已经有序，也需要进行同样多的比较操作。这个特点有时是优势（性能稳定），有时是劣势（无法利用输入的有序性）。
