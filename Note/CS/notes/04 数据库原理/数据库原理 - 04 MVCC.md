...
MVCC（Multi-Version Concurrency Control，多版本并发控制）是一种并发控制机制，通过保存数据的多个版本，让读写操作不互相阻塞。**核心思想**是写操作创建新版本，读操作读旧版本，读写不冲突。

### MVCC 的优势

使用传统锁来控制并发时：读操作需要加 S 锁。写操作需要加 X 锁；读写互斥，并发性能差

使用MVCC：读操作不加锁，直接读快照；写操作创建新版本；读写不互斥，并发性能高
...
- **可见**：说明该版本的数据是在所有活跃事务之前就已提交，因此可以被所有事务读取。

**规则 2**：`TRX_ID >= TRX_ID_MAX`
- **不可见**：如果数据行的 `TRX_ID` 大于等于当前事务的最大事务 ID (`TRX_ID_MAX`)，说明这个数据是在当前事务开始后才创建的，因此对当前事务不可见。

**规则 3**：`TRX_ID_MIN <= TRX_ID < TRX_ID_MAX`

- **需要进一步判断**：
  - **未提交事务判断**：如果该数据行的 `TRX_ID` 在 `TRX_IDs` 列表中，则该数据未提交，因此对当前事务不可见。
  - **已提交事务判断**：如果该数据行的 `TRX_ID` 不在 `TRX_IDs` 列表中（即已提交），则按照事务隔离级别来判断：
       - **RC (Read Committed)**：只要数据已提交，当前事务可以读取。
       - **RR (Repeatable Read)**：当前事务不能看到其他事务在它开始后提交的数据，因此如果该数据是在当前事务开始后提交的，它不可见。
  

**规则 4**：如果当前版本不可见，沿着 `ROLL_PTR` 找到前一个版本，重复上述判断。这是为了处理数据行的历史版本。通过 `ROLL_PTR` 可以追溯到数据行的前一个版本，继续判断其是否对当前事务可见。
