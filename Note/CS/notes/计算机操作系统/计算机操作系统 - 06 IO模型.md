...
特点：编程简单，逻辑清晰但是一个线程只能处理一个连接，需要大量线程处理多个连接，线程切换开销大。适合连接数较少、对实时性要求不高的场景。
...
特点：不会一直阻塞，可以在等待期间做其他事情。但需要不断轮询，消耗 CPU，数据复制阶段仍然阻塞。通常配合 I/O 多路复用使用，很少单独使用。
...
特点：一个线程可以处理多个连接，不需要为每个连接创建线程，适合高并发场景。但编程复杂度较高，数据复制阶段仍然阻塞。
...
特点：不需要轮询，数据准备阶段不阻塞。信号处理机制复杂，数据复制阶段仍然阻塞，实际应用较少。
...
特点：真正的异步，整个过程不阻塞，性能最好。编程复杂，操作系统支持有限（Linux 的 AIO 支持不完善）。
...
|----------|-------------|-------------|---------|---------|
| 同步阻塞 I/O | 阻塞 | 阻塞 | 是 | 是 |
| 同步非阻塞 I/O | 非阻塞（轮询） | 阻塞 | 部分阻塞 | 是 |
| I/O 多路复用 | 阻塞（等待就绪） | 阻塞 | 是 | 是 |
| （同步非阻塞）信号驱动 I/O | 非阻塞 | 阻塞 | 部分阻塞 | 是 |
| 异步 I/O | 非阻塞 | 非阻塞 | 否 | 否 |
