...
REST（Representational State Transfer）是一种接口风格，用于设计网络应用程序的接口。REST API（Application Programming Interface）是基于 REST 原则构建的 Web 服务接口，它允许不同的系统通过 HTTP 协议进行通信和数据交换。其核心理念是客户端与服务端独立，客户端无需知道服务端的服务逻辑，服务端无需知道客户端的用户界面。
