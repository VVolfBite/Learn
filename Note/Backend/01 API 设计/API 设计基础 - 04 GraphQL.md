...
例如，发起一个查询，调用 `user`，传入参数 `id = "1"`，并要求只返回 `id` 和 `name`。
...
GraphQL 的核心是“请求对象上的字段”。查询结构和结果结构非常相似，因此客户端通常能直接预测返回形状。字段也可以返回对象，这时可以继续做次级选择：
...
当同一个字段需要用不同参数查询多次时，直接写会冲突，这时要用别名，别名重命名的是结果中的字段名。
...
GraphQL 支持简写，也支持显式写出操作类型与操作名。操作名称类似程序中的函数名，便于追踪。生产环境更推荐显式写法，便于调试、日志记录和定位问题：
