...
通常我们所说的 OpenAPI 全称应该是 OpenAPI Specification(OpenAPI 规范，简称 OSA)，它使用规定的格式来描述 HTTP [RESTful API](https://zhida.zhihu.com/search?content_id=227981338&content_type=Article&match_order=1&q=RESTful+API&zhida_source=entity) 的定义，以此来规范 RESTful 服务开发过程。使用 JSON 或 YAML 来描述一个标准的、与编程语言无关的 HTTP API 接口。OpenAPI 规范最初基于 SmartBear Software 在 2015 年捐赠的 [Swagger 规范](https://link.zhihu.com/?target=https%3A//swagger.io/resources/open-api/)演变而来，目前最新的版本是 [v3.1.0](https://link.zhihu.com/?target=https%3A//github.com/OAI/OpenAPI-Specification/blob/main/versions/3.1.0.md)。
...
![img](./assets/v2-1a053da86e93ae2a391a5644ad3e15af_1440w.jpg)

### 文档编辑器

文档编辑器方便我们用来编写符合 OpenAPI 规范的文档，有助于提高编写文档的效率，就像 [VS Code](https://link.zhihu.com/?target=https%3A//code.visualstudio.com/) 能够方便我们编写代码一样。文本编辑器推荐使用在线的 [Swagger Editor](https://link.zhihu.com/?target=https%3A//editor.swagger.io/)，能够实现格式校验和实时预览 Swagger 交互式 API 文档功能，效果如下图所示：

![img](./assets/v2-67378d5cd90930b57afd438654203282_1440w.jpg)

如果你习惯使用 `VS Code`，也有[相应插件](https://link.zhihu.com/?target=https%3A//marketplace.visualstudio.com/items%3FitemName%3D42Crunch.vscode-openapi)可供使用。图形编辑器的好处是能够以可视化的形式编辑内容，不了解 OpenAPI 规范语法也能编辑。可以根据自己喜好来进行选择，如 [Stoplight Studio](https://link.zhihu.com/?target=https%3A//stoplight.io/studio)、[APIGit](https://link.zhihu.com/?target=https%3A//apigit.com/zh-CN) 等。

### Mock 服务器

Mock 服务器能够根据所提供的 OpenAPI 接口文档，自动生成一个模拟的 Web Server。使用 Mock 服务器能够轻松模拟真实的后端接口，方便前端进行接口调试。

### 代码生成器

还有一种很实用的工具是代码生成器，代码生成器有两种类型：一种是从代码/注释生成 OpenAPI 文档，另一种是从 OpenAPI 文档生成代码。

这类工具同样非常多，且更为实用。比如我们有一份写好了的 Go Web Server 代码，想要自动生成一份 OpenAPI 文档，就可以使用 [go-swagger](https://link.zhihu.com/?target=https%3A//github.com/go-swagger/go-swagger) 这个工具来生成一份 `openapi.yaml` 文档。
...
不仅 Go 语言有这样的工具，像 [Swagger Codegen](https://link.zhihu.com/?target=https%3A//swagger.io/tools/swagger-codegen/) 和 [OpenAPI Generator](https://link.zhihu.com/?target=https%3A//github.com/openapitools/openapi-generator) 这类工具更是支持几乎所有主流编程语言。

代码生成器是开发者应该着重关注的工具，使用这些工具可以减少大量手动且重复的工作，你可以[在此](https://link.zhihu.com/?target=https%3A//openapi.tools/%23sdk)看下有没有感兴趣的项目供你使用。

### Swagger

Swagger 是一套围绕 OpenAPI 规范所构建的开源工具集，提供了强大和易于使用的工具来充分利用 OpenAPI 规范，Swagger 工具集由最初的 Swagger 规范背后的团队所开发。Swagger 工具集提供了 API 设计、开发、测试、治理和监控等能力，其中最主要的工具包含如下三个：

- [Swagger Codegen](https://link.zhihu.com/?target=https%3A//swagger.io/tools/swagger-codegen/)：根据 OpenAPI 规范定义生成服务器存根和客户端 SDK。
- [Swagger Editor](https://link.zhihu.com/?target=https%3A//swagger.io/tools/swagger-editor/)：基于浏览器的在线 OpenAPI 规范编辑器。
- [Swagger UI](https://link.zhihu.com/?target=https%3A//swagger.io/tools/swagger-ui/)：以 UI 界面的方式可视化展示 OpenAPI 规范定义，并且能够在浏览器中进行交互。

当然 Swagger 也有为企业用户提供的收费版本工具，如 [SwaggerHub Enterprise](https://link.zhihu.com/?target=https%3A//swagger.io/tools/swaggerhub/enterprise/)。

## 以 Go 语言为例的 OpenAPI 在实际开发中的应用
...
这样的工作流看起来似乎没什么问题，不过编写 OpenAPI 文档毕竟是个苦力活，不仅有大量的重复工作，还要求开发者熟练掌握 OpenAPI 规范语法。

### 使用 Swag 生成 Swagger 文档

在 Go 语言生态里，目前有两个比较流行的开源工具可以生成 Swagger 文档，分别是 [go-swagger](https://link.zhihu.com/?target=https%3A//github.com/go-swagger/go-swagger) 和 [swag](https://link.zhihu.com/?target=https%3A//github.com/swaggo/swag)。它们都能根据代码中的注释生成 Swagger 文档，go-swagger 作为一款 OpenAPI.Tools 推荐的工具，其功能比 swag 更加强大且 Github Star 数量也更高。
...
> 注意：以上 `swag init` 命令可选参数介绍略有删减，只列出了常用选项，更完整的文档请参考[官方仓库](https://link.zhihu.com/?target=https%3A//github.com/swaggo/swag/blob/master/README.md%23swag-cli)。
...
### 在 [Gin](https://zhida.zhihu.com/search?content_id=227981338&content_type=Article&match_order=1&q=Gin&zhida_source=entity) 中使用 Swag

在 [gin](https://link.zhihu.com/?target=https%3A//github.com/gin-gonic/gin) 框架能够很方便的使用 swag，步骤如下：
...
以上这些注释最终都会对应到 OpenAPI 2.0 规范的某个字段上。更多说明请参考[官方文档](https://link.zhihu.com/?target=https%3A//github.com/swaggo/swag%23declarative-comments-format)，并且官方也提供了[中文文档](https://link.zhihu.com/?target=https%3A//github.com/swaggo/swag/blob/master/README_zh-CN.md%23%E5%A3%B0%E6%98%8E%E5%BC%8F%E6%B3%A8%E9%87%8A%E6%A0%BC%E5%BC%8F)。
...
![img](./assets/v2-c4b22d42a752c8846feb7ebed44c645a_1440w.jpg)

将 Gin 作为 Swagger UI 服务器

上面我们通过 swag 生成了 Swagger 文档，并手动将生成的 `swagger.yaml` 复制到 Swagger Editor 编辑器进行 Swagger UI 预览。不过这么做显然有点麻烦，好在 swag 作者也考虑到了这一点，所以他又提供了另外两个项目 [gin-swagger](https://link.zhihu.com/?target=https%3A//github.com/swaggo/gin-swagger) 和 [files](https://link.zhihu.com/?target=https%3A//github.com/swaggo/files)，能够直接将 gin 作为 Swagger UI 服务器，这样就不用每次都将 `swagger.yaml` 手动复制到 Swagger Editor 编辑器才能实现 Swagger UI 预览。
...
![img](./assets/v2-866b40cbedfc09575da51151e3c40cfc_1440w.jpg)

这个本地的 Swagger UI 服务器同样支持交互式操作。

展开 `/example/helloworld` 这个接口，点击 `Try it out`。

![img](./assets/v2-19640295ba0d7fdf2667741bdb131a00_1440w.jpg)

接着，点击 `Execute`。

![img](./assets/v2-3501e1ccd3a99c93862ec7ad48b1a2d8_1440w.jpg)

Swagger UI 将会根据文档的 `Base URL` 去请求真正的接口（同时还会给出 cURL 发送请求的命令，方便复制使用），并将响应结果展示出来。

![img](./assets/v2-de99c53af427a0c3b9d480b6ebdbef84_1440w.jpg)

同时后端服务器能够打印出请求记录：

![img](./assets/v2-46a76840c84c9a44480c2689cb50704c_1440w.jpg)

与前端对接时，我们只需要将接口文档地址给到前端，前端就可以根据这个 Swagger UI 界面进行接口查阅和调试了，非常方便。

### 让 Swag 支持多版本 API 文档

实际工作中，我们的项目会比这个只有一个接口的 demo 复杂得多，同时 API 也可能会支持多版本，比如 `/api/v1`、`/api/v2`，针对这些场景，我实现了一个使用示例 [swag-example](https://link.zhihu.com/?target=https%3A//github.com/jianghushinian/swag-example)，供你参考。
...
也许相较于 Swagger UI 多年不变的界面风格，你更喜欢 ReDoc 风格的 UI，那么 [go-redoc](https://link.zhihu.com/?target=https%3A//github.com/mvrilo/go-redoc) 是一个比较不错的选择。
...
![img](./assets/v2-f350c31c7bcbdd054f920b8422b9f8ef_1440w.jpg)

不过，相较于 Swagger UI，Redoc UI 有个弊端是不能实现交互式操作，如果仅以此作为文档查阅工具，没有交互式操作的需求，那么还是比较推荐使用的。

### 更先进的 API 工具

除了 OpenAPI.Tools 推荐的开源工具，社区中其实还有很多其他优秀工具值得尝试使用，比如我这里要推荐的一款国产工具 [Apifox](https://link.zhihu.com/?target=https%3A//www.apifox.com/)，官方将其定义为 Apifox = Postman + Swagger + Mock + JMeter，集 API 设计/开发/测试 于一身。

Apifox 可谓一站式图形化工具，其功能非常强大，就像前文提到的 APIGit 同时具备了编辑器和 Mock 服务器的功能，Apifox 有过之而无不及。

![img](./assets/v2-d30f446e1e2eb4dca3f99a921c10942c_1440w.jpg)

图形化工具上手难度不大，加上 Apifox 本身由国内开发，非常容易上手，所以本文也就不深入介绍了，你可以观看官方教程 [21 分钟学会 Apifox](https://link.zhihu.com/?target=https%3A//www.apifox.cn/help/%23_21-%E5%88%86%E9%92%9F%E5%AD%A6%E4%BC%9A-apifox-%F0%9F%91%8D) 来学习使用。
